...
function VAST_Voronoi(bbox) {
    ...
    // tree map of positions going x -> y
    var posTree = {};
    ...
    var l_insert = this.insert = function (id, pos) {
        ...
        if (!posTree.hasOwnProperty(pos.x)) {
            posTree[pos.x] = {};
            posTree[pos.x][pos.y] = 0;
        }

        posTree[pos.x][pos.y] = id;
    ...
    var l_remove = this.remove = function (id) {
        ...
        invalidated = true;
        if (posTree.hasOwnProperty(sites[id].x)) {
            delete posTree[sites[id].x][sites[id].y];

            if (Object.keys(posTree[sites[id].x]).length == 0) {
                delete(posTree[sites[id].x]);
            }
        }
    ...
    var get_bound = this.get_bound = function (id, radius, x,y) {
        ...
        for (var key in sites){
            if (!withinAOI(sites[key],idx,radius, x,y)){
        ...
        */
        ...
        for (j=0; j < result.cells.length; j++) {
            ...
            for (k=0; k < voronoi_section.halfedges.length; k++){
                ...
                if (!withinAOI(voronoi_section.halfedges[k].edge.va,idx,radius, x,y) || !withinAOI(voronoi_section.halfedges[k].edge.vb,idx,radius, x,y)) {
    ...
    */
    ...
    function withinAOI(pos, id, radius, pos_x, pos_y) {
        var dx = pos.x-pos_x;
        var dy = pos.y-pos_y;
    ...
    var _isOverlapped = function (pos, id) {
        if (posTree.hasOwnProperty(pos.x)) {
            if (posTree[pos.x].hasOwnProperty(pos.y)) {
                if (posTree[pos.x][pos.y] != id) {
                    return true;
                }
            }
        }
