...
function VAST_Voronoi(bbox) {
    ...
    if (bbox === undefined)
        bbox = {xl:-10000, xr:10000, yt:-10000, yb:10000};
    ...
    // NOTE: current check allows non-boundary neighbors but not fully enclosed ones to pass this test
    this.is_boundary = function (id, pos, radius) {
        ...
        // NOTE: cannot use simply not enclosed to test, as many other nodes will
        // qualify as boundary sites
        return !enclosed(idx, pos, radius);
    }
    ...
    this.get_bound = function (id, radius, mouse_x,mouse_y) {
        ...
        for (i=0; i < result.cells.length; i++){
            ...
            if (i !== idx){
                if (withinAOI(result.cells[i].site,idx,radius, mouse_x.value,mouse_y.value)){
                    AOI_list.push(result.cells[i]);
                }
            }
        }
        ...
        for (j=0; j < AOI_list.length; j++) {
            var voronoi_section = AOI_list[j];
            for (k=0; k < voronoi_section.halfedges.length; k++){

                if (voronoi_section.halfedges[k].va == undefined) {
                }
                //check to see if a vertex is out of the AOI
                if (!withinAOI(voronoi_section.halfedges[k].edge.va,idx,radius, mouse_x.value,mouse_y.value) || !withinAOI(voronoi_section.halfedges[k].edge.vb,idx,radius, mouse_x.value,mouse_y.value)) {
                    bound_list.push(AOI_list[j]);
                    break;
                }
            }
        }
    ...
    var withinAOI = this.withinAOI = function (pos, id, radius, pos_x, pos_y) {
        var dxx = pos.x;
        var dyy = pos.y;
        var dx = dxx-pos_x;
        var dy = dyy-pos_y;
    ...
    */
    ...
    function getNeighborSet(cell_idx) {
        ...
        if (cell_idx >= 0 && cell_idx < result.cells.length) {
