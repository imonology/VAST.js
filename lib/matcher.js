...
function matcher(isGateway, host, port, x, y, radius, onJoin) {
	...
	var GSaddr = {hostname : 'LAPTOP-JJ5440PB.local', port : 7777};
	...
	var _init = function(callback){
		...
		_vonPeer.init((isGateway ? VAST.ID_GATEWAY : VAST.ID_UNASSIGNED),_addr.host, _GWaddr.port, _that, function (local_addr) {
			_addr = local_addr;
	...
	var l_handlePacket = this.handlePacket = function(from_id, pack){
		...
		switch (pack.type) {
            ...
            case Matcher_Message.PUB: {
				...
				_sendPublication(pub, true); // Allow forwarding
            }
            break;
            ...
            case Matcher_Message.PUB_MATCHED: {
				...
				_sendPublication(msg, false); // Disallow furter forwarding
            }
            break;
		...
		_io.on('connection', function(socket){
            ...
            socket.on('client_info', function(info){
                ...
                if(_id == VAST.ID_GATEWAY && (info.clientID == -1 || info.matcherID == VAST.ID_UNASSIGNED)){
                    ...
                    _connectionCount++;
                    var clientID = info.hostname + '-' + _connectionCount; // multiple clients on same host
				}
	...
	// forwardFurther sets weter the mathcer should foward the publication to 
	// externally mathcered subscriptions.  
	var _sendPublication = function(publication, forwardFurther){
		var pointPacket;
			...
			for (var j in clientSubs){
				...
				if(sub.aoi.intersectsArea(pub.aoi)){
					...
					// the nearest recipient to the subscription
						...
						// Am I allowed to forward this publication, and am I not sending the 
						// publication to the same mathcer multiple times?

						if (forwardFurther === true && matcherTargets.includes(sub.hostID) === false){
