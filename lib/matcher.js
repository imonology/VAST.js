...
function matcher(isGateway, host, port, x, y, radius, onJoin) {
	...
	var l_handlePacket = this.handlePacket = function(from_id, pack){
		...
		switch (pack.type) {
            ...
            case Matcher_Message.PUB_FORWARD : {
				...
				console.log(_id + ': Publication Forward from matcher '+pack.src);
				console.log(msg);
            ...
            case Matcher_Message.SUB_OVERLAP : {
				console.log(_id + ': Sub Overlap from matcher: '+pack.src);
				console.log(msg);
				var sub = msg;
			...
			case Matcher_Message.SUB_DISTANT : {
				console.log(_id + ': Sub Distant from matcher '+pack.src);
				console.log(msg);

				var sub = msg;

				// Subscription is to a distant area (further than enclosing and boundary neighbours)
				// I am not the owner and I don't contain the center (vonPeer.contains() is a broken function)
				//if (sub.hostID !== _id && vonPeer.contains(_id, sub.aoi.center) === false){

			//	console.log(vonPeer.closest_to(sub.aoi.center))

				if (sub.hostID !== _id && vonPeer.closest_to(sub.aoi.center) != _id){
			...
			//		console.log(_id + ': Sub Distant push');
			//		console.log(pack);
				...
				// I contain the center of the subscription
				if (sub.hostID !== _id && vonPeer.closest_to(sub.aoi.center) == _id){
					_addSubscription(sub, false);
				}
            }
	...
	var _subscribe = function(clientID, x, y, radius, channel){
		...
		// The sub is not in my area, must forward it to nearest neighbour
		//if (vonPeer.contains(_id, sub.aoi.center) == false){

		if (vonPeer.closest_to(sub.aoi.center) != _id){
			// add myself to chain
			sub.propagationChain.push(_id);

			// forward message to nearest neighbour
			var pack = new VAST.matcherPack(Matcher_Message.SUB_DISTANT, sub, _id);
			pack.targets = [];
			pack.targets.push(vonPeer.closest_to(sub.aoi.center));
			vonPeer.matcherMessage(pack);
		}
	...
	var _addSubscription = function (sub, is_owner) {
        ...
        //    _updateSubscription(new_sub.subID,sub.aoi);
			...
			//add the overlapping neighbours as recipients
			for (var i = 0; i < overlapping.length; i++){

				// subscription has already been sent
				if(new_sub.recipients.includes(overlapping[i]))
				continue;

				/*
				//if j is a neighbour to i, check if they are closer than we are
				for(var j = 0; j < new_sub.recipients.length; j++){
					
				}
				*/

				new_sub.recipients.push(overlapping[i]);
				targets.push(overlapping[i]);
			}
			...
			if (targets.length > 0){
				...
				var pack = new VAST.matcherPack(Matcher_Message.SUB_OVERLAP, new_sub, _id);
				pack.targets = targets;
				vonPeer.matcherMessage(pack);
			}	

			// Record a new subscription and add myself as a recipient
