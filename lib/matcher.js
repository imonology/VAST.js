...
function matcher(isGateway, host, port, x, y, radius, onJoin) {
	...
	var _init = function(callback){
		...
		_vonPeer = new Worker('./lib/VON_peer_worker.js', {workerData});
	...
	var _onWorkerMessage = function(message, callback){
		console.log('main process recieved message: ', message);
		switch (message.type){
			// VON peer is inited
	...
	var _sendWorkerMessage = function(type, packet){
		// send the message to the worker (VON peer)
				...
				if(sub.aoi.intersectsArea(pub.aoi)){
					...
					// the nearest recipient to the subscription
						...
						if (pub.recipients.includes(sub.hostID) === false && matcherTargets.includes(sub.hostID) === false){
							...
							// will only forward if I am closest to the host between all recipiencts
							_sendWorkerMessage(Worker_Message.POINT_MESSAGE_REC, pointPacket);
