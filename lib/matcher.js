...
=======
>>>>>>> 03ee812c23058e9073880b6bb57645a98c44db5b
	...
	var _initListeners = function(){
		...
		_io.on('connection', function(socket){
			...
			socket.on('publish', function(data) {
				...
				var matcher_pub_requests = (''+UTIL.getTimestamp()+', '+data.x+', '+data.y+', '+data.radius+', '+data.payload+', '+data.channel+', '+clientID+', '+"matcher-"+_id+'\n');

				result = lib.LogToFile('./logs/GW_publicationrequests.txt', matcher_pub_requests)
				// console.log(clientID, data.x, data.y, data.radius, data.payload, data.channel)
...
<<<<<<< HEAD
				var matcher_subscription_list = (''+UTIL.getTimestamp()+', '+msg.x+', '+msg.y+', '+msg.radius+', '+clientID+', '+msg.channel+', '+_id+' \n');

				// var matcher_data_header = ('timestamp, x, y, r, clientID, channel, MatcherID \n'); //header for file : time, AOI, type, channel (if subscription)
				result = lib.LogToFile('./logs/GW_matchersublist.txt', matcher_subscription_list);

				// log subscriber list to file (Gateway/Matcher Logs here)
				
				// result = lib.LogToFile('./logs/GW_matchersublist-2.txt', msublist)

				console.log('Matcher '+_id+' is receiving this request : ', UTIL.getTimestamp(), clientID, msg.x, msg.y, msg.radius, msg.channel, _id, '\n')
				

				// console.log('this is subsription list : ', _subscriptions)
...
>>>>>>> 03ee812c23058e9073880b6bb57645a98c44db5b
	...
	var _sendPublication = this.sendPublication = function(publication, allowForwarding){
		...
		for (var clinetID in _subscriptions) {
			clientSubs = _subscriptions[clinetID];
	...
	var _startRecordingBandwidth = function(){
		recBandwidth.init('matcher-'+_id + '-bandwidth','/results');
			...
			bytesSent = 0;
						...
						// Todo Debo : record the requests to a different file

						// var matcher_requests = (' '+UTIL.getTimestamp()+', '+x+', '+y+', '+r+', '+type+' \n');

						// result = lib.LogToFile('matcher_log.txt', pub_data_header)
