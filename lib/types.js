...
var l_region = exports.region = function (site, halfEdges, boundarySize, closeMe) {
    ...
    this.boundaryEdges = [];
    this.boundarySize = boundarySize || 20;
    ...
    this.convertingEdges = function () {
        ...
        for (var i=0; i < this.halfedges.length; i++) {
            ...
            point = _extendPoint(pt1,pt2, this.boundarySize);
            point2 = _extendPoint(pt3,pt2, this.boundarySize);

            var finalX = point.x+point2.x+pt2.x;
            var finalY = point.y+point2.y+pt2.y;
    ...
    var _extendPoint = this.extendPoint = function(pt1,pt2, boundarySize) {
        var dx = pt2.x-pt1.x;
        var dy = pt2.y-pt1.y;

        var m,c, yBar, xBar;
        if (dx != 0 && dy != 0) { //case 1 - sloped line
            m = dy/dx;
            c = pt2.y - m*pt2.x;
            yBar = m*(pt2.x+(dx/(Math.abs(dx)))*boundarySize) + c;
            xBar = pt2.x + (dx/(Math.abs(dx)))*boundarySize;
        } else if (dx == 0 && dy != 0) { // case 2 - vertical line
            xBar = pt2.x;
            yBar = pt2.y + (dy/(Math.abs(dy)))*boundarySize;
        } else { // case3 - horizontal line
            xBar = pt2.x + (dx/(Math.abs(dx)))*boundarySize;
            yBar = pt2.y;
        }
...
var l_matcher = exports.match = function (id, endpt, time, boundarySize, region) {
    ...
    this.region.boundarySize = boundarySize || 40;
