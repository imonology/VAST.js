...
var l_region = exports.region = function (site, halfEdges, boundarySize, closeMe) {
    ...
    var _getBoundaryVertex = function(pt1,pt2,pt3, boundarySize) {
        console.log("Points")
        console.log(pt1);
        console.log(pt2);
        console.log(pt3);
        ...
        if (dx != 0) { // line 1 vertical
            ...
            console.log("case 3");
                    alpha = m1 == 0 || m2 == 0 ? Math.PI - Math.atan(Math.abs((m2-m1)/(1+m2*m1))) : Math.atan(Math.abs((m2-m1)/(1+m2*m1)));

                    theta = m1 == 0 ? alpha/2 : Math.PI-alpha/2;
                } else {
            console.log("case 4");
                    alpha = Math.PI/2;
            ...
            console.log("case 5");
                alpha = m1 < 0 ? Math.PI/2+Math.atan(Math.abs(m1)) : Math.PI/2+Math.atan(m1);
                theta = Math.atan(Math.abs(m1)) + alpha/2;

                if (pt3.x-pt1.x < 0) {
                    theta = Math.atan(Math.abs(m1)) + alpha/2 + Math.PI;
                } else {
                    theta = m1 != 0 ? Math.atan(Math.abs(m1)) - alpha/2 : Math.atan(Math.abs(m1)) + alpha/2;
                }
            }
        } else {
            console.log("case 6");
