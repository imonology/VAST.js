...
var l_matcherPack = exports.matcherPack = function(type, msg, sender){
    ...
    // The source chain (for returning messages to the source / unconnected peers)
...
var l_sub = exports.sub = function (hostID, clientID, subID, channel, aoi) {
    ...
    this.propagationChain = [];        // chain of matchers to send to (if not an enclosing neighbour of the owner matcher)
    ...
    this.update = function (info) {
        ...
        this.client_id = (info.client_id !== 0 ? info.client_id : this.client_id);
        this.sub_id = (info.sub_id !== undefined ? info.sub_id : this.sub_id);
    ...
    this.parse = function (js_obj) {
        try {
            ...
            this.client_id = parseInt(js_obj.client_id);
            this.sub_id = parse(js_obj.sub_id);
    ...
    // print out node info
    this.toString = function () {
        return '[' + this.id + '] host_id: ' + hostID + ' layer: ' + this.layer + ' AOI: ' + this.aoi.toString();
    }
}
