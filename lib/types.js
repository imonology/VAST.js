...
var l_region = exports.region = function (site, halfEdges, boundarySize, closeMe) {
    ...
    this.boundaryEdges = [];
    this.boundarySize = boundarySize || 5;
    console.log("Begin convert edges");
    this.convertedEdges = [];
    console.log("End convert edges");
    ...
    this.convertingEdges = function () {
        console.log("Start edge conversion");
        var edgeList = [];
        var boundaryList = [];
        var pt, pt2;
        var point, point2;
        ...
        for (var i=0; i < this.halfedges.length-1; i++) {
            ...
            point = this.extendPoint(pt);
            console.log("1.5");

            pt2 = this.halfedges[i+1].getStartpoint();
            console.log("2");

            point2 = this.extendPoint(pt2);
            console.log("2.5");

            // create line segment
            boundaryList.push(new segment(new point2d(point.x,point.y), new point2d(point2.x, point2.y)));
            console.log("3");
        ...
        if (this.halfedges.length != 0) {
            ...
            boundaryList[boundaryList.length] = new segment(new point2d(point2.x, point2.y),boundaryList[0].p1);

            // closing off the voronoi section with the last line that connects to the start of the first line
            edgeList[edgeList.length] = new segment(new point2d(pt2.x, pt2.y),edgeList[0].p1);

            // assign boundary list to boundary edges attribute of this object
            this.boundaryEdges = boundaryList;
        }
        console.log(boundaryList);

        console.log("end edge conversion");

        return edgeList;
    }

    this.intersects = function (center, radius) {
        console.log("Region intersection calculation");
        ...
        if (typeof this.convertedEdges[0].intersects !== 'function') {
            console.log("error in halfedges. Converting them");
    ...
    this.within = function(pos) {
        console.log("within");
        var verty = [];
        var vertx = [];
        var pt1;

        for (var i=0; i < this.boundaryEdges.length; i++) {

            pt1 = this.boundaryEdges[i].pt1;
        ...
        var nvert = this.boundaryEdges.length;
        var i, j, c = 0;
    ...
    this.extendPoint = function(point) {
        console.log("extend point");
        var dx = point.x-this.site.x;
        var dy = point.y-this.site.y;

        var m = dy/dx;
        var c = point.y - m*point.x;

        var dirX = dx/Math.abs(dx)*this.boundarySize;

        var yBar = m*(point.x+dirX) + c;
        var xBar = (yBar-c)/m;
        ...
        pt.x = dx == 0 ? point.x : point.x + dx/Math.abs(dx)*this.boundarySize;
        pt.y = dy == 0 ? point.y : point.y + dy/Math.abs(dy)*this.boundarySize;
...
var l_matcher = exports.match = function (id, endpt, time, boundarySize, region) {
    ...
    this.region.boundarySize = boundarySize || 5;
...
var l_client = exports.connClient = function (id, aoi, time, connected) {
    this.id = id;
