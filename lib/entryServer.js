...
function entryServer (startPort) {
    ...
    var _loadBalanceShift = function (matcherID, en_list, pos) {
        ...
        for (var key in Object.keys(en_list)) {
            ...
            var movement = _getMovement(neighbourPos, pos, true);
            LOG.debug(movement);
            if (!movement){
                continue;
            }
    ...
    var _getMovement = function(target, destination, minDistance) {
        LOG.debug("Getting movement of matcher");
        var dx = (target.center.x-destination.center.x)/MOVEMENT_SPEED;
        var dy = (target.center.y-destination.center.y)/MOVEMENT_SPEED;

        if (Math.abs(dx*MOVEMENT_SPEED) <= MINIMUM_DISTANCE && Math.abs(dy*MOVEMENT_SPEED) <= MINIMUM_DISTANCE) {
        ...
        if (minDistance) {
            ...
            if (Math.pow(x,2) + Math.pow(y,2) <= Math.pow(MINIMUM_DISTANCE,2)) {
                var ratio;
                if (diffY != 0) {
                    ratio = diffX/diffY;
                    x = destination.center.x + ratio*MINIMUM_DISTANCE/Math.sqrt(Math.pow(ratio,2) + 1);
                    y = destination.center.y + MINIMUM_DISTANCE/Math.sqrt(Math.pow(ratio,2) + 1);
                } else if (diffX != 0) {
                    ratio = diffY/diffX;
                    x = destination.center.x + MINIMUM_DISTANCE/Math.sqrt(Math.pow(ratio,2) + 1);
                    y = destination.center.y + ratio*MINIMUM_DISTANCE/Math.sqrt(Math.pow(ratio,2) + 1);
                }
    ...
    var _shiftMatcher = function (matcherID,pos) {
        ...
        LOG.debug(matchers[matcherID].position);
        LOG.debug(matchers[matcherID].origin);
        ...
        _io.on("connection", function(socket) {
            ...
            socket.on("normalise", function(matcherID, neighbours) {
                ...
                for (var i=0; i< neighbours.length;i++) {
                    ...
                    movement = _getMovement(neighbourPos, neighbourOrigin, false);
                    if (!movement)
                        continue;
