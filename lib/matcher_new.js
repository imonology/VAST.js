...
const CLIENT_THRESHOLD = 55;         // maximum number of clients that can be connected to the matcher
const SUB_THRESHOLD = 60;          // the maximum number of subscriptions before asking neighbours for help
...
function VAST_matcher(von_peer) {
    ...
    */
    ...
    var _connHandler = this.connHandler = function (type,data) {
        ...
        switch (type) {
            case "ID": {
                LOG.debug("matcher::ID => Ready to send ID to entry server");

                var aoi = new VAST.area(new VAST.pos(_self.region.site.x, _self.region.site.y), 0);
