...
const CLIENT_THRESHOLD = 5;     // maximum number of clients that can be connected to the matcher
const PENDING_INTERVAL = 1000;  // currently 1 second. subject to change
const TICK_RATE = 50;           // tick rate for visualiser
const BOUNDARY_SIZE = 20         // size of boundary threshold before transferring clients
...
function VAST_matcher(von_peer) {
    ...
    var _move = this.move = function (clientID, aoi, channel, hostID) {
        ...
        if (_self.region.within(aoi.center)) {
    ...
    var _updateSubscription = function(sub_ID, new_aoi, sendtime, is_owner) {
        ...
        if (_self.region.intersects(new_aoi.center, new_aoi.radius)) {
            ...
            for (var keys in _neighbours) {
                ...
                LOG.layer(_neighbours[keys].region.convertedEdges);
    ...
    var _tick = function() {
        if (countdown != 0) {
            ...
            if (_self.region.boundaryEdges.length == 0 && _self.region.halfedges.length != 0) {
                ...
                _self.region.convertingEdges();
                console.log(_self.region);
            }

            console.log("matcher::tick => boundary regions");
            console.log(_neighbours[_self.id].region.boundaryEdges);
            console.log(_neighbours[_self.id].region.halfedges);
    ...
    var _connectPending = function () {
        if (Object.keys(pendingClients).length > 0) {
            ...
            if (numClients <= CLIENT_THRESHOLD) {
                ...
                var pendClient = pendingClients.pop();
                LOG.layer(pendClient);
                var type = "join";
                var aoi = {
                    x: pendClient.aoi.center.x,
                    y: pendClient.aoi.center.y,
                    radius: pendClient.aoi.radius
                }
                var data = {
    ...
    */
    ...
    var _insertMatcher = this.insertMatcher = function (matcher) {
        ...
        var before = util.getTimestamp();
        console.log(_voro.getRegion(matcher.id));
        matcher.region.update(_voro.getRegion(matcher.id));
        console.log(matcher.region);
        console.log("Before edge conversion");
        matcher.region.convertingEdges();
        var after = util.getTimestamp();
        ...
        switch (type) {
            ...
            case "join": {
                ...
                if (numClients <= CLIENT_THRESHOLD) {
                    ...
                    pendingClients.push({
                        client: new VAST.connClient(clientID, new VAST.area(new VAST.pos(data.aoi.x, data.aoi.y), data.aoi.radius),time, false),
                        username: data.username,
                        socketID: data.socketID,
                        type: data.type,
                        newConnection: data.newConnection,
