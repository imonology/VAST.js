...
function VAST_matcher(von_peer) {
    ...
    var _leave = this.leave = function (clientID) {
        ...
        for (var key in _client2subID[clientID]) {
        ...
        switch (pack.type) {
            ...
            case Matcher.ID: {
                if (_is_gateway) {
                    ...
                    LOG.layer("matcher::packetHandler::ID => ID: " + _clientID + " packID: " + pack.msg.clinetID, _self.id);

                    // match local ID to incoming if incoming is larger than local ID
                    if (pack.msg.clientID > _clientID) {
                    ...
                    if (pack.msg.clientID > _clientID) {
                        LOG.layer("matcher::packetHandler::ID => clientID received from gateway: " + pack.msg.clientID + ". Replacing current ID of " + _clientID, _self.id);
                        _clientID = pack.msg.clientID;

                        // decrease the local ID by one after assigning so that when connHandler::type increases it, it will match the received ID.
                        _clientID--;
                    } else {
                        LOG.layer("matcher::ID => received ID of " + pack.msg.clientID + " which is less than or the same as current clientID " + _clientID + ". Ignore it", _self.id);
                    }
