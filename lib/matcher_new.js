...
const CLIENT_THRESHOLD = 10;         // maximum number of clients that can be connected to the matcher
...
const BOUNDARY_SIZE = 10            // size of boundary threshold before transferring clients
...
function VAST_matcher(von_peer) {
    ...
    var _setJoined = function () {
        ...
        // start normalising interval function
        setInterval(_normalise, NORMALISE_INTERVAL);
    ...
    var _connectPending = function () {
        if (Object.keys(pendingClients).length > 0) {
            ...
            if (numClients < CLIENT_THRESHOLD) {
                ...
                var type = "join";
                var aoi = {
                    x: pendClient.client.aoi.center.x,
                    y: pendClient.client.aoi.center.y,
                    radius: pendClient.client.aoi.radius
                }
                var data = {
                ...
                if (_self.region.within(pendClient.client.aoi.center)) {
                    ...
                    if (subID == "") {
                        ...
                        pendingClients.push(pendClient);
                        pendingClientsMap[pendClient.client.id] = pendClient.client;
                    }
                    else {
    ...
    */
        ...
        switch (type) {
            ...
            case "join": {
                ...
                if (numClients < CLIENT_THRESHOLD) {
                    ...
                    pendingClients.push({
                        client: new VAST.connClient(clientID, new VAST.area(new VAST.pos(data.aoi.x, data.aoi.y), data.aoi.radius),time),
                        username: data.username,
                        socketID: data.socketID,
                        type: data.type,
                        newConnection: data.newConnection
    ...
    var visualComm = this.visualComm = function (type, message) {
        console.log("Received message in visualComm")
        switch (type) {
            case "voro": {
