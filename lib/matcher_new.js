...
function VAST_matcher(von_peer) {
    ...
    */
    ...
    var _connHandler = this.connHandler = function (type,data) {
        ...
        switch (type) {
            case "type": {
                LOG.layer("matcher::connHandler => received connection of type: " + data.type, _self.id);
                if (_is_gateway) {
            ...
            case "join": {
                LOG.layer("matcher::join => received username " + data);
                _clientID++;

                // create space for client info to be stored
                connections[_clientID] = data.username;
