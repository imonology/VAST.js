...
function VAST_matcher(von_peer) {
    ...
    var _move = this.move = function (clientID, aoi, channel, hostID) {
        ...
        LOG.layer("matcher::move => client closest to matcher [" + matcherID + "]");

        // check if the movement takes the subscription outside of this matcher's region
        if (matcherID != _self.id) {
    ...
    */
    ...
    var _checkOverlap = function (sub) {
        ...
        if (overlap.length > 0) {
            ...
            sub.recipients = sub.recipients.concat(overlap);
    ...
    */
    ...
    var _addSubscription = function (sub, is_owner) {
        ...
        if (_subscriptions.hasOwnProperty(sub.subID)) {
            LOG.layer("macther::addSubscription => subscription already exists. Update instead");
            var date = new Date();
    ...
    var _updateSubscription = function(sub_ID, new_aoi, sendtime, is_owner) {
        ...
        if (is_owner && !sub.recipients.includes(sub.host_id))
        ...
        var recipients = [];
        LOG.layer("matcher::updateSubscription => region");
        LOG.layer(_self.region.halfedges);
        // create region edges
        // TODO: Does the region need to be updated if there are already edges?
        if (_self.region.halfedges.length == 0) {
            var before = util.getTimestamp();
            _self.region.update(_voro.getRegion(_self.id));
            _self.region.convertEdges();
            var after = util.getTimestamp();
            LOG.layer("matcher::updateSubscription => time taken to do edge retrieval and conversion: " + (after - before), _self.id);
        }

        if (_self.region.intersects(new_aoi.center, new_aoi.radius)) {
            ...
            for (var keys in _neighbours) {
                ...
                LOG.layer(_neighbours[keys].region.halfedges);
                if (_neighbours[keys].region.intersects(new_aoi.center, new_aoi.radius) && !sub.recipients.includes(parseInt(keys))) {
        ...
        if (is_owner) {
            ...
            if (recipients.length != 0) {
                ...
                sub.recipients = sub.recipients.concat(recipients);
                _sendMessage(recipients, Matcher.SUB_NOTIFY, sub, VAST.priority.HIGHEST, true, LAYER.MATCHER);
                _subscriptions[sub.subID] = sub;
            }
        }
    }
    ...
    */
    ...
    var _tick = function() {
        if (countdown != 0) {
            ...
            _visualReturn("matcher", {matcher:_self.region.site, clients:_clients, id: _self.id});
        } else {
    ...
    // TODO: create interval function that does the reconnecting of pending clients
    // this includes setting the interval and removing it when there are no more pending clients
    ...
    */
    ...
    var _updateMatcher = this.updateMatcher = function (id,position) {
        ...
        // updating my own position
            _self.region.update(_voro.getRegion(id));
        } else {
    ...
    var _connHandler = this.connHandler = function (type,data) {
        ...
        switch (type) {
            ...
            case "join": {
                ...
                LOG.layer("matcher::connHandler => number of clients connected to matcher: " + numClients, _self.id);
                if (numClients <= CLIENT_THRESHOLD) {
                    numClients++;
                    ...
                    _clients[clientID] = new VAST.connClient(clientID, new VAST.area(new VAST.pos(data.aoi.x, data.aoi.y), data.aoi.radius),time, true);
                    ...
                    pendingClients[clientID] = new VAST.connClient(clientID, new VAST.area(new VAST.pos(data.aoi.x, data.aoi.y), data.aoi.radius),time, false);

                    var en_list = _voro.get_en(_self.id);

                    var returnData = {
                        matcherID: _self.id,
                        aoi: _self.region.site,
                        en_list: en_list
                    }
            ...
            case "matcherMove": {
                ...
                var sendList = [];
                for (var key in _neighbours) {
                    sendList.push(paseInt(key));
                }

                LOG.layer("matcher::matcherMove => sending movement to: ", _self.id);
                LOG.layer(sendList);

                _sendMessage(sendList, Matcher.MOVE, data, VAST.priority.HIGHEST,true,LAYER.MATCHER);
            }
                break;
            default:
    ...
    */
    ...
    // list of clients awaiting connection to this matcher when threshold is reached
    var pendingClients = {};
