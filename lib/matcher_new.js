...
// config
var DEFAULT_RADIUS = 1000; // default radius given to a subscription that does not supply one.
...
function VAST_matcher(von_peer) {
    ...
    var _sub = this.sub = function (clientID,aoi, channel, type) {
        LOG.layer("matcher::sub => subscribing " + clientID + " to " + aoi.toString() + " for channel " + channel);
        ...
        // create sub object
        var sub = new VAST.sub(_self.id, clientID, channel, aoi, type);
    ...
    var _unsub = this.unsub = function (clientID, channel, username){
        ...
        var sub = new VAST.sub(_self.id, clientID, channel, aoi, connections[clientID]);
    ...
    var _pub = this.pub = function (clientID, player, aoi, message, channel, packetName, oldTargets) {
        ...
        //LOG.layer("matcher::pub => aoi:");
        //LOG.layer(aoi);

        LOG.layer("Subscription list: ");
        ...
        for (var key in _subscriptions) {
            ...
            if (sub.aoi.covers(aoi)) {
                ...
                // if we are not the host of the sub, then send the publication to that host for them to process
                if (sub.host_id != _self.id) {
                    ...
                    LOG.layer("matcher::pub => self ID does match host_id");
                    ...
                    if (!clientSend.hasOwnProperty(sub.id) && sub.id != clientID) {
                        LOG.layer("matcher::pub => adding " + sub.id + " to the sending list");
    ...
    var _move = this.move = function (clientID, aoi, channel) {
        ...
        var subID = _generateSubID(clientID, _self.id, channel);
    ...
    */
    ...
    var _updateSubscription = function(sub_ID, new_aoi, sendtime, is_owner) {
        ...
        var updateSub = new VAST.sub(hostID, sub.id, sub.layer, new_aoi, connections[sub.id], sendtime);
    ...
    var _connHandler = this.connHandler = function (type,data) {
        ...
        switch (type) {
            case "type": {
                ...
                if (_is_gateway) {
                    LOG.layer("matcher::connHandler::type => received connection from server. Start accepting client requests.", _self.id);
                    _clientID++;
                    ...
                    LOG.layer("matcher::connHandler => server connected to me but I am not the gateway. Redirect to: " + _gwaddr.toString(), _self.id);
                    _clientReturn("gateway", _gwaddr);
                }
            }
                break;
            ...
            case "subscribe": {
                ...
                var clientID = data.username == "client" ? (_clientID +1) : data.clientID;
                ...
                _sub(clientID, aoi, data.channel, type);
            }
                break;
            ...
            case "move": {
                ...
                var aoi = new VAST.area(new VAST.pos(data.x,data.y),data.radius);
                _move(data.clientID, aoi, data.channel);
            }
                break;
            ...
            case "publish": {
                ...
                _pub(data.clientID, data.username, aoi, data.payload, data.channel, data.packetName);
            }
                break;
            default:
