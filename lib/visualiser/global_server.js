...
function globalServer(){
    ...
    var _initialiseListeners = function(){
        ...
        _io.on('connection', function(socket){
            ...
            socket.on('matcher_join', function(){
                socket.emit('request_info');
            });

            socket.on('matcher_info', function(info){
                console.log(info);
            ...
            socket.on('matcher_update', function(data){
                ...
                if(data.id !== -1 && typeof(data.id) !== 'undefined'){
                    ...
                    for (var key in data.subscriptions){
                        sub.parse(data.subscriptions[key])
                        tempSubs[key] = sub;
                    }

                    data.subscriptions = tempSubs;
