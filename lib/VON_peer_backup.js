...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    this.forwardOverlapping = function(areaPacket, callback){
        ...
        var chain = Object.values(pack.msg.chain); // list of nodes the message came from
        ...
        for (var j = 0; j < newNeighbours.length; j++){
            ...
            // For each overlapping neighbour, check distance
            for (var k = 0; k < overlapping.length; k++){
                tempNode = _getNode(overlapping[k]);
                tempDist = targetPos.distance(tempNode.aoi.center);
                if (tempDist === minDist){
                    minID = tempNode.id < minID ? tempNode.id : minID;
                }
        ...
        //console.log('recNeighbours, new Neighbours, recipients, targets');
        //console.log(recNeighbours);
        //console.log(newNeighbours);
        //console.log(recipients);
        //console.log(VON_pack.targets);
...
// forward message to all overlapping neighbours
    this.forwardOverlapping = function(areaPacket, callback){
        ...
        // list of nodes that are currently forwarding the message
        var currentNodes = Object.values(areaPacket.currentNodes);

        // path of nodes the message came from
        var chain = Object.values(areaPacket.chain); 

        var newNeighbours = [];  // array of overlapping neighbours who don't have the message yet 
        var currNeighbours = []; // array of my neighbours that currently have the message
        ...
        for (var i = 0; i < overlapping.length; i++){
            ...
            if (!currentNodes.includes(id) && !recipients.includes(id)){
                newNeighbours.push(id);
            
            // current nodes are not neccessarily my neighbours
            } else if(currentNodes.includes(id)){
                currNeighbours.push(id);
            }
        }

        console.log('my id:' + _self.id);
        console.log('currNeighbours, new Neighbours, recipients, targets, chain');
        ...
        for (var j = 0; j < newNeighbours.length; j++){
            ...
            // For each current neighbour, check distance
            for (var k = 0; k < currNeighbours.length; k++){
                tempNode = _getNode(currNeighbours[k]);
                tempDist = targetPos.distance(tempNode.aoi.center);
                if (tempDist === minDist){
                    minID = tempNode.id < minID ? tempNode.id : minID;
                }
            ...
            // Am I closer than any other current neighbour?
            if(minID === _self.id){
                targets.push(targetNode.id);
            }
        }

        // set up matcher packet for next round of forwarding
        currentNodes = newNeighbours;
        chain.push(_self.id);

        areaPacket.recipients = recipients.concat(currentNodes);
        areaPacket.currentNodes = currentNodes;
        areaPacket.chain = chain;
