...
var _analyseData = function(steps, onDone) {
    console.log("Analysing drift distance...");
    var mean = [];
    var driftMean = 0;
    var results = [];

    var neighbourAverage = undefined;
    ...
    for (var i = 1; i < Object.keys(steps).length; i++) {
        ...
        for (var clientKey in steps[i]) {
            ...
            neighbourAverage = neighbourAverage == undefined ? neighbourCount : (neighbourAverage+neighbourCount)/2;
    ...
    console.log("Average neighbour per client: " + neighbourAverage);
