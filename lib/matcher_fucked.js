...
const { pos, clientInfo } = require('./types');
...
function matcher(isGateway, host, port, x, y, radius, onJoin) {
		...
		switch (pack.type) {
            ...
            case Matcher_Message.FIND_MATCHER: {
				...
				if(_clientID2socket[msg.clientID] !== undefined){
					...
					_updateVisualiser(visualMessage.CLIENT_UPDATE);
				} 
	...
	var _initialiseListeners = function(){
		...
		GS.on('connection', function(){
			...
			console.log('sending my info');
			console.log(info.subscriptions);

			GS.emit('matcher_info', info);
		});

		GS.on('request_info', function(){
			var info ={
				id : _id,
				pos : _pos,
				aoi : _aoi,
	...
	var _updateVisualiser = function(msg){
		...
		console.log('sending update to visualiser')
		console.log(data.subscriptions);

		GS.emit('matcher_update', data);
	}
