...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    this.forwardOverlapping = function(areaPacket, callback){
        ...
        var commonNeighbours, checkNeigbours, nearest;
        var id, rootDist, rootPos, myRootDistance;
        ...
        for (var key in _neighbors){
            id = _neighbors[key].id;

            // skip myself and any neighbours that have already recieved the message
            if(_self.id == id || recipients.includes(id)){
                continue
            }

            region.update(_voro.getRegion(id));
        ...
        for (var j = 0; j < newNeighbours.length; j++){
            ...
            for (var i = 0; i < checkNeigbours.length; i ++){
                tempNode = _getNode(checkNeigbours[i]);                
        ...
        // The new neighbours are now also recipients
        recipients.push(newNeighbours);
