...
require("appdynamics").profile({
 controllerHostName: 'city201811222340539.saas.appdynamics.com',
 controllerPort: 443,

 // If SSL, be sure to enable the next line
 controllerSslEnabled: true,
 accountName: 'city201811222340539',
 accountAccessKey: 'kvg25srnv3vm',
 applicationName: 'VONPeer',
 tierName: 'Global viewer',
...
// config
var VON_DEFAULT_PORT        = 37700;    // by default which port does the node listen
var AOI_DETECTION_BUFFER    = 0;        // detection buffer around AOI
...
var MOVEMENT_SPEED          = 0.5;      // the amount a client moves in a single movement frame
var MOVEMENT_FRAME          = 40;       // the amount of time between each movement interation for the visualiser
var MOVEMENT_GLOBAL         = 1000      // tick rate for the sending of information to the global visualiser
...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    this.init = function (self_id, port, localIP, entryServers, onDone, initMatcher) {
        //LOG.info('VON PEER init called', _self.id);

        //LOG.debug("Entry servers: " + entryServers);

        self_id = self_id || VAST.ID_UNASSIGNED;
        port = port || VON_DEFAULT_PORT;

        _initMatcher = initMatcher;

        // create new layer
        var handler = new msg_handler(self_id, port, localIP, entryServers, function (local_addr) {

            // NOTE: this will cause initStates() be called
            handler.addHandler(_that, LAYER.VON);
    ...
    var _query = this.query = function (contact_id, center, msg_type, msg_para) {
        ...
        //LOG.debug('VON_peer query() will contact node [' + contact_id + '] to find acceptor', _self.id);
        ...
        _sendMessage(contact_id, VON_Message.VON_QUERY, msg, VAST.priority.HIGHEST, true, LAYER.VON);
    }

    // join a VON network with a given aoi
    var _join = this.join = function (GW_addr, aoi, onDone, onMove) {

        // check if already joined
        if (_state === VAST.state.JOINED) {
            //LOG.warn('VON_peer.join(): node already joined', _self.id);
            if (typeof onDone === 'function')
                onDone(_self.id);
            return;
        }

        //_getID accessed _self_id from vast_net.js->msg_handler->init
        //LOG.debug('VON_peer join() called, self: ' + _self.id + ' getID: ' + _getID(), _self.id);
        ...
        //NOTE: This is where message IDs start getting weird -> try play around here (was VAST.ID_GATEWAY)
        _storeMapping(VAST.ID_GATEWAY, addr);
        //LOG.warn('gateway set to: ' + addr.toString(), _self.id);

        if (aoi === undefined) {
            aoi = new VAST.area();
        }

        //LOG.info("Position set to " + aoi.toString(), _self.id);
        ...
        // keep reference to call future once movement is initiated
        _move_onDone = onMove;
        ...
        if (_getID() === VAST.ID_UNASSIGNED) {
            _sendMessage(VAST.ID_GATEWAY, VON_Message.VON_HAIL, {request: true}, VAST.priority.HIGHEST, true, LAYER.VON);
        }
        else
            _setInited();
    }
    ...
    var _leave = this.leave = function () {
        ...
        //LOG.debug('VON_peer leave() called, neighbor size: ' + Object.keys(_neighbors).length, _self.id);
        ...
        if (_visual_id !== undefined) {
            clearInterval(_visual_id);
            _visual_id = undefined;
        }
    }

    // move the AOI to a new position (or change radius)
    var _move = this.move = function (aoi, sendtime) {
        //LOG.warn('VON_Peer ['+_self.id+'] moving to: ' + aoi.center.toString(), _self.id);

        try {
            if (typeof aoi.radius === 'string')
                aoi.radius = parseInt(aoi.radius);
            else if (typeof aoi.radius !== 'number') {
                //LOG.error('radius format incorrect, ignore move request', _self.id);
                return _self.aoi;
            }
        }
        catch (e) {
            //LOG.error('parsing radius error: ' + e, _self.id);
            return _self.aoi;
        }
        ...
        if (aoi_reshaped) {
            LOG.warn('AOI radius updated to: ' + aoi.radius, _self.id);
        }

        if (_isOverlapped(aoi.center, _self.id)) {
            //LOG.debug("Overlap detected", _self.id);
            aoi.center.update(_adjustPos(aoi.center, _self.id));
        }
        _self.aoi.update(aoi);
        ...
        if (_self.id == VAST.ID_GATEWAY) {
            ...
            for (key in _neighbors) {
                neighbour_pos[key] = _neighbors[key].aoi.center;
            }

            var state = new VAST.clientState(_self.id,UTIL.getTimestamp(), _self.aoi.center,neighbour_pos, latMax, latMin, latAve, bandwidth, latencies);

            recorder.write(state, _self.id);
        }

        //LOG.debug('AoI + center', _self.id);
        //LOG.debug(_self.aoi, _self.id);
        //LOG.debug(_self.aoi.center, _self.id);
        ...
        // check to make sure that self is not undefined in _neighbors
        if (_neighbors[_self.id] === undefined) {
            //LOG.warn("self information in neighbours was lost for " + _self.id + ". Reinstate")
            _neighbors[_self.id] = _self;
        }
        ...
        //LOG.warn("Regular neighbours:", _self.id);
        //LOG.warn(regular_list, _self.id);
        pack.targets = regular_list;
        pack.type = (aoi_reshaped ? VON_Message.VON_MOVE_F : VON_Message.VON_MOVE);
        _sendPack(pack, true);

        //LOG.warn("Boundary Neighbours:", _self.id);
        //LOG.warn(boundary_list, _self.id);
        ...
        if (now - _lastCheckConsistency > 1000) {
            //console.log('calling _checkConsistency from move function for ' + _self.id);
            _checkConsistency();
        }
    ...
    // get a list of relevant neighbors
    this.list = function () {
        return _neighbors;
    }
    ...
    var _put = this.put = function (obj) {
        _meta = obj;
        //LOG.debug('after put() meta keys: ' + Object.keys(_meta).length, _self.id);
    ...
    var _get = this.get = function () {
        //LOG.debug('get() meta keys: ' + Object.keys(_meta).length, _self.id);
        return _meta;
    }
    ...
    // get a call to the message handler
    ...
    //set debug mode
    this.debug = function(halt) {
        _halt = JSON.parse(halt);
        progress = _halt ? false : true;
    }
    ...
    var _isRelevantNeighbor = function (node1, node2) {
        ...
        /*
        //LOG.warn("Relevancy: "+node1.id+" "+node2.id);
        //LOG.warn(_voro.get_sites());
        //LOG.warn(_voro.is_enclosing(node1.id, node2.id));
        //LOG.warn(_isAOINeighbor(node1.id, node2));
        //LOG.warn(_isAOINeighbor(node2.id, node1));
        */
    ...
    var _isOverlapped = function (checkPos, checkID) {
        ...
        for (var id in _neighbors) {
            var pos = _neighbors[id].aoi.center;
            if (pos.equals(checkPos) && id != checkID) {
                //LOG.warn("Overlap detected", _self.id);
                return true;
            }
        }
    ...
    var _adjustPos = function (pos, id) {
        do {
            ...
            pos = _bound(pos);
            //LOG.debug("Position adjusted to ("+pos.x+","+pos.y+")", _self.id);
    ...
    var _insertNode = function (node) {
        ...
        //LOG.debug('insertNode [' + node.id + '] isNeighbor: ' + _isNeighbor(node.id), _self.id);
    ...
    var _updateNode = function (node) {
        ...
        // check to see if neighbour is still relevant
        if (!_isRelevantNeighbor(_self,node)) {
            _sendBye([node.id]);
            return true;
        }
    ...
    // send out ping messages to receive latency
    var _ping = function() {
        ////LOG.debug("Send Pings", _self.id);
        //var targets = [];
        //for (var key in _neighbors)
            //_sendMessage(key, VON_Message.VON_PING, {}, VAST.priority.HIGH, true, LAYER.VON);
    }

    // the function to check if there are messages to step through
    var _step = function () {
        try {
            if (messages.length != 0) {
                progress = true;
                var obj = messages.pop();
                _packetHandler(obj.from_id, obj.pack);
            }
        }
        catch (e) {
            LOG.error('tick error:\n' + e.stack, _self.id);
    ...
    var _calcLatency = function (pack) {
        // calculate maximum latency
        latMax = (pack.latency > latMax) ? pack.latency : latMax;

        if (pack.latency >= 0) {
            // calculate minimum latency
            latMin = (pack.latency < latMin) ? pack.latency : (latMin == 0) ? pack.latency : latMin;

            // calculate average latency
            // network latency
            latTotal += pack.latency;
            latCount++;
            latAve = latTotal/latCount;

            // forwarding latency
            latencies['forwardTime'][0] += (pack.forwardTime-pack.receiveTime);
            latencies['forwardTime'][1] = latencies['forwardTime'][0]/latCount;

            // processing specific pack's latency
            if (latencies['processTime'][pack.type] == undefined) {
                latencies['processTime'][pack.type] = {
                    0:0,    // total latency for average calculation
                    1:0,    // number of packets of this type
                    2:0     // average latency
                }
            }
            latencies['processTime'][pack.type][0] += (pack.processTime-pack.forwardTime);
            latencies['processTime'][pack.type][1]++;
            latencies['processTime'][pack.type][2] = latencies['processTime'][pack.type][0]/latencies['processTime'][pack.type][1];
        }
    }
    ...
    var _setInited = function () {
        ...
        // send message to visualiser telling it to initialise the ID
        _visualReturn("ID",_self.id);
        ...
        _visualComm("voro_client", "Start the global tick");
        setInterval(_ping, 750);

        if (typeof _initMatcher === 'function')
            _initMatcher(_that, _self.id, addr.port);
        ...
        //LOG.info('_self.id:'+_self.id+' ID_GATEWAY:'+VAST.ID_GATEWAY, _self.id);
        if (_self.id === VAST.ID_GATEWAY)
            _setJoined();
        else
    ...
    var _setJoined = function () {
        ...
        // initialise record keeper
        recorder.init(_self.id, _self.aoi.radius);
        ...
        // start ticking
        //LOG.info("Tick has started", _self.id);
    ...
    var _contactNewNeighbors = function () {
        ...
        for (var i=0; i < new_list.length; ++i) {
            ...
            if (_isRelevantNeighbor(node, _self)) {
                ...
                //LOG.debug('[' + node.id + '] is relevant to self [' + _self.id + ']', _self.id);
        ...
        // need to first insert all nodes before notifying of neighbours,
        // else information is based off of incorrect knowledge of what client can see
        for (var i=0; i < relevant_neighbors.length; i++) {
            // NOTE: even if en_list is empty, should still send out HELLO
            // empty can occur if initially just two nodes with overlapped positions
            _sendHello(relevant_neighbors[i].id);
            //console.log('_sendEN called in _contactNewNeighbors for ' + _self.id + ' for relevant neighbour ' + relevant_neighbors[i].id);
            _sendEN(relevant_neighbors[i].id);
        }

        //LOG.layer("Sending information to matcher");
        //console.log("relevant_neighbors")
        //console.log(relevant_neighbors)
        _sendMessage(_self.id, Matcher.NODE, relevant_neighbors, VAST.priority.NORMAL, true, LAYER.MATCHER);

        // NOTE: erase new neighbors seems to bring better consistency
        //       (rather than keep to next round, as in previous version)
        _new_neighbors = {};

        //LOG.debug('total neighbors (after process neighbors): ' + Object.keys(_neighbors).length, _self.id);
    ...
    var _checkNeighborDiscovery = function (check_requester_only, check_en_only) {
        ...
        if (check_requester_only === true)
            requesters = _req_nodes;    //boundary neighbours
        else
        ...
        for (var from_id in requesters) {
            ...
            for (var i=0; i < check_list.length; i++) {
                ...
                if (state != NeighborState.NEIGHBOR_REGULAR) {
                    ...
                    /*
                    *    previous state is not overlapped AND is now overlapped (achieves 2)
                    *                                    OR
                    *    previous state is not enclosed AND now enclosed (achieves 1)
                    */

                    if ((known_state & NeighborState.NEIGHBOR_OVERLAPPED) == 0 &&
                        ((state & NeighborState.NEIGHBOR_OVERLAPPED) > 0 || ((known_state & NeighborState.NEIGHBOR_ENCLOSED) == 0 &&
                        (state & NeighborState.NEIGHBOR_ENCLOSED) > 0)))
                        notify_list.push(id);
    ...
    var _checkConsistency = function (skip_id) {
        //console.log('_sendEN called in _checkConsistency for ' + _self.id);
        //set _lastCheckConsistency to now
        var now = UTIL.getTimestamp();
        _lastCheckConsistency = now;
    ...
    var _removeNonOverlapped = function () {
        ...
        var grace_period = now + (MAX_DROP_SECONDS);
        // NOTE: perhaps this time drop is too large?
        ...
        for (var id in _neighbors) {
            ...
            ////LOG.warn("Time drop for id: "+id+" is "+_time_drop[id]);
        ...
        ////LOG.debug("Delete list: "+delete_list);
    ...
    var _sendNodes = function (target, list, reliable) {
        ...
        //LOG.debug('_sendNodes: [' + _self.id + '] sends to [' + target + '] ' + list.length + ' nodes', _self.id);

        // we warn but do not prevent send (this is needed when gateway joins itself as client)
        if (list.length === 0) {
            LOG.warn('sendNodes send out a list of 0 neighbors', _self.id);
        }
        ...
        _sendMessage(target, VON_Message.VON_NODE, nodes, VAST.priority.NORMAL, reliable, LAYER.VON);
    }
    ...
    var _sendHello = function (target) {
        ...
        //LOG.debug('meta keys: ' + Object.keys(_meta).length, _self.id);
        ...
        if (Object.keys(_meta).length > 0) {
            //LOG.debug('storing meta keys ' + Object.keys(_meta).length + ' to node to be sent for VON_HELLO', _self.id);
            node.meta = _meta;
        }

        // prepare HELLO message
        // TODO: do not create new HELLO message every time? (local-side optimization)
        _sendMessage(target, VON_Message.VON_HELLO, node, VAST.priority.HIGHEST, true, LAYER.VON);
    }
    ...
    var _sendEN = function (target) {
        ...
        if (id_list.length > 0)
            _sendMessage(target, VON_Message.VON_EN, id_list, VAST.priority.HIGH, true, LAYER.VON);
    }
    ...
    var _sendBye = function (targets) {
        ...
        if (targets.length > 0) {
            ...
            //LOG.debug('sendBye target size: ' + targets.length, _self.id);

            var pack = new VAST.pack(VON_Message.VON_BYE, {}, VAST.priority.HIGHEST, LAYER.VON);
            pack.targets = targets;
            _sendPack(pack, true);

            // perform node removal
            for (var i=0; i < targets.length; i++) {
                //LOG.debug('removing node: ' + targets[i], _self.id);
    ...
    var _assignNewID = function () {
        ...
        //LOG.warn('new ID assigned: ' + _new_ID, _self.id);

        //TODO: Check whether ID incrementation is necessary
        return _new_ID++;
    }

    var _packetHandler = this.packetHandler = function (from_id, pack) {
        var now = UTIL.getTimestamp()
        if (pack.forwardTime == 0 || now > pack.receiveTime)
            pack.forwardTime = now;


        // store message in buffer unless given
        if (!progress) {
            var obj = {from_id: from_id, pack: pack};
            //LOG.warn("message was stored for later", _self.id)
            messages.push(obj);
            _visualReturn("empty", false);
            return true;
        } else {
            LOG.warn("Processing message", _self.id);
        }

        var action = new VAST.action(pack.type, VON_Message_String[pack.type], from_id, pack.targets, pack.msg, "incoming");
        _visualReturn("Message received", action);

        // if node is not joined, do not process any message, unless it's response for
        // getting ID (VON_HAIL) or neighbor list (VON_NODE) or handshake (VON_HELLO)
        // NOTE: it's possible for VON_HELLO to arrive earlier than VON_NODE
        /*
        if (_state < VAST.state.JOINED) {

            if (pack.type !== VON_Message.VON_HAIL &&
        ...
        */
        ...
        if (_self.id === VAST.ID_UNASSIGNED && pack.type !== VON_Message.VON_HAIL) {
            //LOG.error('VON_peer: node not yet init (got unique ID), should not process any messages', _self.id);
            return false;
        }

        //LOG.debug('[' + _self.id + '] ' + VON_Message_String[pack.type] + ' from [' + from_id + '], neighbor size: ' + Object.keys(_neighbors).length, _self.id);

        switch (pack.type) {

            // VON's ping, to check if a connected host is still alive
            case VON_Message.VON_HAIL: {
                ...
                if (typeof pack.msg.request !== 'undefined' && pack.msg.request == true) {
                    ...
                    //LOG.warn('assign new ID [' + remote_id + '] to [' + from_id + ']', _self.id);
                    ...
                    if (_self.id === VAST.ID_UNASSIGNED) {
                        //LOG.warn('first ID assigned, likely I am the gateway', _self.id);
                        _self.id = remote_id;
                    }

                    ////LOG.warn('VON_HAIL receive request, simply respond');
                    _sendMessage(from_id, VON_Message.VON_HAIL, {request: false, aid: remote_id}, VAST.priority.HIGH, true, LAYER.VON);
                }
                // otherwise we got a response from gateway, set my ID, can now send join request
                    ...
                    //LOG.debug('assigned_id: ' + assigned_id, _self.id);
                    _setID(assigned_id);
                    _setInited();
                }
            ...
            case VON_Message.VON_QUERY: {
                // check to make sure that the positional information is there
                if (pack.msg.pos === undefined) {
                    //LOG.error("pos is undefined in VON_QUERY. Query request denied");
                    _disconnect(pack.src);
                    return false;
                }
                ...
                //LOG.debug('VON_QUERY checking pos: ' + pos.toString(), _self.id);

                // find the node closest to the joiner
                var closest = _voro.closest_to(pos);
                //LOG.debug('closest node: ' + closest + ' (' + typeof closest + ')', _self.id);
                ...
                if (closest === null) {
					LOG.warn('closest node is null, something is not right.. please check', 'VON_QUERY', _self.id);
				}

				if (closest !== null &&
					!_voro.contains(_self.id, pos) &&
                     !_isSelf(closest) &&
                    closest != from_id) {
                    ...
                    //LOG.debug('accepting VON_QUERY from: ' + from_id, _self.id);

                    // I'm the acceptor, re-direct message content to self
                    pack.type = pack.msg.type;
                    pack.msg = pack.msg.para;
                    pack.sendTime = UTIL.getTimestamp();
                    pack.receiveTime = UTIL.getTimestamp();
                    pack.layer = LAYER.VON;
                    pack.targets = [];
                    ...
                    //LOG.debug('redirect to self: ' + JSON.stringify(pack), _self.id);
            ...
            case VON_Message.VON_JOIN: {
                // check the message to ensure that the information is there
                if (pack.msg.id === undefined ||
                    pack.msg.endpt === undefined ||
                    pack.msg.aoi === undefined) {
                        //LOG.error("packet_handler: joiner node has invalid data: "+pack.msg);
                ...
                //LOG.debug('joiner: ' + joiner.toString(), _self.id);

                // TODO: verify message in a systematic way
                if (joiner.endpt.addr.isEmpty()) {
                    //LOG.error('joiner has no valid IP/port address, ignore request', _self.id);
                    break;
                }
                ...
                /* possibly do not need this due to local change in _insertNode
                // TODO: Implement a way to check against a certain position,
                // as this joining client will most certainly overlap with _self.pos
                if (_isOverlapped(joiner.aoi.center, joiner.id)) {
                    //LOG.debug("joiner client overlap. Adjusting position", _self.id);

                    var centre = _adjustPos(joiner.aoi.center, joiner.id);
                    joiner.aoi.center.update(centre);
                    _updateNode(joiner);
                    _sendMessage(joiner.id, VON_Message.VON_POS, {pos: joiner.aoi.center}, VAST.priority.HIGH, true, LAYER.VON);
                    break;
                }
                */
                ...
                if (Object.keys(_neighbors).length === 1) {
                    //LOG.warn('gateway getting its own VON_QUERY, return empty list', _self.id);
                ...
                for (var id in _neighbors) {
                    ...
                    //LOG.debug('is relevant: ' + is_relevant, _self.id);
                    //LOG.debug('neighbour.id: '+neighbor.id, _self.id);
                    //LOG.debug('joiner.id: '+joiner.id, _self.id);
                ...
                //LOG.debug('notify ' + list.length + ' nodes to joiner: ' + out_str, _self.id);
                if (list.length <= 1) {
                    LOG.warn('too few neighbors are notified! check correctness\n', _self.id);
                    //LOG.debug(_voro.to_string(), _self.id);
                }

                // send a list of nodes to a specific target node
                _sendNodes(joiner.id, list, true);

                // check to see if there are any neighbours that are no longer neighbours
                _removeNonOverlapped();
            }
            break;

            case VON_Message.VON_POS: {
                // extract message
                var pos = new VAST.pos();

                if (pack.pos === undefined) {
                    //LOG.warn("packet_handler: pos undefined in VON_POS. Readjusting position locally");
                    pos = _adjustPos(_self.aoi.center, _self.id);
                } else {
                    pos.parse(pack.msg.pos);
                }

                //LOG.info("changing position to ("+pos.x+','+pos.y+")", _self.id);
                _self.aoi.center.update(pos);
                //LOG.debug(_self.aoi.center, _self.id);
                _updateNode(_self);

                // resend the join message with new position
                _sendMessage(from_id, VON_Message.VON_JOIN, _self, VAST.priority.HIGH, true, LAYER.VON);
            }
            ...
            case VON_Message.VON_NODE: {
                ...
                for (var i=0; i < nodelist.length; i++) {
                    ...
                    if (nodelist[i].id === undefined ||
                        ...
                        nodelist[i].aoi === undefined) {
                            //LOG.error("packet_handler: nodelist node has invalid data: "+nodelist[i].msg);
                            // TODO: create error message that gets information again
                            break;
                    }
                    ...
                    //LOG.debug('node ' + new_node.toString(), _self.id);
                    ...
                    */
                    ...
                    if (_new_neighbors.hasOwnProperty(new_node.id) === false ||
                        ...
                        //LOG.debug('adding node id [' + new_node.id + '] type: ' + typeof new_node.id, _self.id);
                ...
                //console.log('calling _contactNewNeighbors from VON_NODE for ' + _self.id);
                _contactNewNeighbors();
            ...
            case VON_Message.VON_HELLO: {
                ...
                var node = new VAST.node();
                node.parse(pack.msg);
                //LOG.debug('node: ' + node.toString(), _self.id);

                // store meta data, if any
                if (pack.msg.hasOwnProperty('meta') && typeof pack.msg.meta === 'object') {
                    //LOG.debug('VON_HELLO storing to node meta keys: ' + Object.keys(pack.msg.meta).length, _self.id);
                ...
                _sendMessage(from_id, VON_Message.VON_HELLO_R, res_obj, VAST.priority.HIGH, true, LAYER.VON);

                //LOG.debug("VON_peer::VON_HELLO => sending a 'HELLO' message to the matcher.");

                // notify own matcher of node that has been discovered
                _sendMessage(_self.id, Matcher.HELLO, node, VAST.priority.HIGH, true, LAYER.MATCHER);

                // check if enclosing neighbors need any update
                //console.log('calling _checkConsistency from VON_Hello for ' + _self.id);
            ...
            case VON_Message.VON_HELLO_R: {
                ...
                if (_neighbors.hasOwnProperty(from_id) === true) {
                    // check to see that information in message has come through correctly

                    // update the neighbor's position
                    // NOTE: 'time' is not updated here
                    var neighbor = _neighbors[from_id];

                    // check to see that information is not corrupted in any way
                    try {
                        if (!neighbor.aoi.center.parse(pack.msg.pos))
                            throw "error";
                    } catch (e) {
                        //LOG.error("packet_handler: position could not be read in VON_HELLO_R. Resend hello message");
                        _sendHello(pack.src);
                        break;
                    }
                    ...
                    // store meta data, if any
                    if (pack.msg.hasOwnProperty('meta') && typeof pack.msg.meta === 'object') {
                        //LOG.debug('meta keys received: ' + Object.keys(pack.msg.meta).length, _self.id);
                        neighbor.meta = pack.msg.meta;
                    }

                    //LOG.debug('got latest pos: ' + neighbor.aoi.center.toString() + ' id: ' + from_id, _self.id);
                    try {
                        _updateNode(neighbor);
                    }
                    catch(e) {
                        //LOG.debug(e.stack, _self.id);
                    }
                }
                else {
                    //LOG.warn('[' + _self.id + '] got VON_HELLO_R from unknown neighbor [' + from_id + ']', _self.id);
                }
            }
            break;
            ...
            case VON_Message.VON_EN: {
                //console.log("[" + _self.id + "] Receiving VON_EN message from " + from_id);
                ...
                if (_neighbors.hasOwnProperty(from_id) === false) {
                    //LOG.debug('neighbor [' + from_id + '] is no longer connected, ignore VON_EN request', _self.id);
                    break;
                }

                // extract list of EN id received & create searchable list of EN
                var id_list = pack.msg;
                //LOG.debug('recv en_list size: ' + id_list.length, _self.id);
                ...
                var en_list = _voro.get_en(_self.id);
                ////LOG.warn("Enclosing neighbour list versus client: "+pack.src);
                ////LOG.warn(id_list);
                ////LOG.warn(en_list);
                ...
                for (var i=0; i < en_list.length; ++i)
                {
                    ...
                    // NOTE: check for neighbours that other nodes think we should know about
                    ...
                    if (id != from_id &&
                        ...
                        //LOG.debug("Pushing " + id + " to missing list");
                        missing.push(id);
                    }
                }
            ...
            case VON_Message.VON_MOVE_FB: {
                ...
                // extract full node info or just position update and ensure that
                // information is not missing
                var node = new VAST.node(from_id);
                try {
                    if (!node.parse(pack.msg))
                        throw "error";
                } catch (e) {
                    //LOG.error("packet_handler: position not valid in VON_MOVE_FB");
                    return false;
                }

                if (_isOverlapped(node.aoi.center, node.id)) {
                    //LOG.debug("Position being adjusted locally to avoid overlap", _self.id);
                    node.aoi.center.update(_adjustPos(node.aoi.center, node.id));
                }
                ...
                // //LOG position of neighbours

            }
            break;

            // VON's response to a ping to determine round-trip time
            case VON_Message.VON_PONG: {
                var ping = pack.ping;

                //LOG.debug("Ping from [" + from_id + "] is " + pack.ping, _self.id);

                latencies['ping'][0] += ping;
                latencies['ping'][1]++;
                latencies['ping'][2] = latencies['ping'][0]/latencies['ping'][1];
            }
            break;
            ...
            case VON_Message.VON_BYE: {
                ...
                if (_neighbors.hasOwnProperty(from_id)) {
                    //console.log('calling _checkConsistency from VON_BYE for ' + _self.id);
                    _checkConsistency(from_id);
                    _deleteNode(from_id);
                }
                ...
                //LOG.debug('disconnect success: ' + result, _self.id);
                //LOG.debug('after removal, neighbor size: ' + Object.keys(_neighbors).length, _self.id);
            }
            break;

            case VON_Message.VON_SHIFT: {
                //LOG.info("VON_peer::VON_SHIFT => Moving to respond to matcher's request", _self.id);

                var matcherAoI = new VAST.area(new VAST.pos(pack.msg.x, pack.msg.y), _self.aoi.radius);

                var date = new Date();
                var time = date.getTime();

                _move(matcherAoI, time);
            }
            break;
        ...
        pack.processTime = UTIL.getTimestamp();

        // update latency values
        _calcLatency(pack);

        // reset progress variable if told to _halt
        if (_halt) {
            progress = false;
        } else if (messages.length != 0) {  // loop through the rest of the messages if there are any
            var tempHolder = messages.pop();
            progress = true;
            _packetHandler(tempHolder.from_id, tempHolder.pack);
        }

        // send a message to the visualiser to inform it whether there are still messages to process or not
        if (messages.length!=0){
            _visualReturn("empty", false);
        } else {
            _visualReturn("empty", true);
        }

        // update bandwidth if the first update comes through (should come through every second)
        if (pack.bandwidth != 0) {
    ...
    var _connHandler = this.connHandler = function (id) {
        //LOG.debug('VON peer [' + id + '] connected', _self.id);
    }

    var _disconnHandler = this.disconnHandler = function (id) {
        //LOG.debug('VON peer [' + id + '] disconnected', _self.id);
    ...
    // channel for communication with the visualiser
    var _visualComm = this.visualComm = function (type,message) {
        //LOG.info("Type: "+type, _self.id);
        //LOG.info(message, _self.id);
        var _id = _getID();
        //LOG.info("ID: "+_id, _self.id);

        switch (type) {

            // send voronoi information
            case "voro": {
                if (_visual_id == undefined) {
                    //LOG.info("Visualiser tick has started", _self.id);
                    _visual_id = setInterval(function(voro, neighbours, halt) {
                        _visualReturn("voro",[voro.get_sites(),JSON.stringify(neighbours), halt]);
                    }, MOVEMENT_FRAME, _voro, _neighbors, _halt);
                }
            }
            break;

            case "voro_client": {
                if (_global_id == undefined) {
                    //LOG.info("Global view tick has started", _self.id);
                    //LOG.warn(_voro.get(_id), _self.id);
                    _global_id = setInterval(function(voro, id) {
                        _visualReturn(type, [voro.get(id), id]);
                    }, MOVEMENT_GLOBAL, _voro, _id);
                }
            }
            break;

            // synchronised movement starter
            case "sync_move": {
                //LOG.info("Start synchronised movement", _self.id);
                if (typeof _move_onDone === 'function') {
                    _move_onDone(_self.id);
                }
            }
            break;
            ...
            case "move": {
                if (_getID() !== VAST.ID_GATEWAY) {
                    var target = new point2d(message.center.x, message.center.y);
                    var radius = message.radius;

                    // teleport
                    var pos =  new VAST.area(new VAST.pos(target.x, target.y), radius);

                    /*
                    //teleport move
                    _move(pos);
                    //LOG.info("Target has been reached");
                    */

                    /*
                    // incremental movement
                    clearInterval(moveInterval);       // clear the old interval if there is any

                    // create movement towards target destination at a set interval
                    moveInterval = setInterval(function(target, radius){
                        var dist = target.distance(_self.aoi.center);
                        var dx = target.x-_self.aoi.center.x;
                        var dy = target.y-_self.aoi.center.y;
                        var ratio = dist/MOVEMENT_SPEED;
                        ...
                        // if the node has not reached the target destination then move towards it
                        if (dist > 0.1) {
                            var pos =  new VAST.area(new VAST.pos(_self.aoi.center.x+dx, _self.aoi.center.y+dy), radius);
                            _move(pos);
                        } else {
                            //LOG.info("Target has been reached");
                            _checkConsistency();
            ...
            case "disconnect": {
                //LOG.debug("Visualiser tick stopped", _self.id);
                clearInterval(_visual_id);
                _visual_id = undefined;
                _global_id = undefined;
            }
            break;

            default: {
                LOG.error("VON_peer: visual communicator does not understand type '"+type+"'", _self.id);
            }
            break;
        }
    }
    ...
    var _initStates = this.initStates = function (msg_handler) {
        ...
        //LOG.debug('initStates called', _self.id);
    ...
    // constructor
    //
    //LOG.debug('VON_peer constructor called',-1);
    ...
    var _storeMapping, _getID, _setID, _disconnect, _sendMessage, _sendPack, _visualReturn, _initMatcher;
    ...
    // callback used to start synchronised movement
    var _move_onDone = undefined;
    var _startMovement = false;

    // interval id for removing periodic ticking
    var _interval_id = undefined;

    // interval id for visualiser tick
    var _visual_id = undefined;

    // interval id for global visualiser tick
    var _global_id = undefined;
    ...
    var moveInterval = undefined;   // interval for the movement function in the visualiser

    var _lastRemoveNonOverlapped;   // last timestamp when removeNonOverlapped is called
    var _lastCheckConsistency;      // last timestamp when removeNonOverlapped

    // create an object to calculate Voronoi diagram for neighbor mangement/discovery
    var _voro;

    // init stat collection for selected message types
    var _stat;

    // init message collection for message types
    var _actions;
    ...
    //debug stuff
    ////
    // decide whether to progress or not
    var progress;

    // debug boolean to check whether to halt after every message or not
    var _halt;
    ...
    var latencies = {
        ...
        'ping' : {
            0:0,    // total ping
            1:0,    // number of ping calculations
            2:0     // average ping
        }
    };
