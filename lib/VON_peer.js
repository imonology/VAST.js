...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    // NOTE: currently used to check whether to process a message or not
    ...
    var _packetHandler = this.packetHandler = function (from_id, pack) {
        ...
        */
        ...
        // send updated voronoi diagram to visualiser
        _visualReturn('voro',[_voro.get_sites(),JSON.stringify(_neighbors)]);

        // reset progress variable if told to halt
        if (halt) {
            progress = false;
        } else if (messages.length != 0) {  // loop through the rest of the messages if there are any
            var holder = messages.pop();
            _packetHandler(holder.from_id, holder.pack);
        }
    ...
    // boolean to check whether to halt every message or not
    var halt = true;
