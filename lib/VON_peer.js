...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    // This function sends a point message only if I am the 
    // nearest recipient to the target
    this.pointMessageRecipients = function(pointPacket, recipients){ 
        ...
        for (var i = 0; i < recipients.length; i++){
            ...
            key = recipients[i].toString(); //I know... strong types would be nice
            ...
            if (tempDist === minDist){
                minID = neighbour.id < minID ? neighbour.id : minID;
            }
    ...
    var _forwardOverlapping = function(areaPacket, callback){
        ...
        var newNeighbours = {};
        var newNeighbourKeys = [];   

        var targets = [];
        
        // Need to convert voronoi cell to VAST region for intersect checking (slow)
        ...
        for (var key_i in _neighbours){
            ...
            // If the neighbour's region intersects, add them to the list
            region.update(_voro.getRegion(id_i));
            region.convertingEdges();
            if(region.intersects(aoi.center, aoi.radius)){
                newNeighbours[key_i] = neighbour_i;
                newNeighbourKeys.push(key_i);
            }
        }

        var commonNeighbourKeys = []; // array of neighbour keys (IDs) we have in common
        ...
        for (var i = 0; i < newNeighbourKeys.length; i++){

            neighbour_i = newNeighbours[newNeighbourKeys[i]];
            id_i = neighbour_i.id;

            // get a list of the target's neighbours (according to our record of neighbours)
            commonNeighbourKeys = _voro.get_neighbour(id_i, neighbour_i.aoi.radius);
            
            nearest = true; // I am nearest to the root (for now)

            // for each common neigbour
            for (var j = 0; j < commonNeighbourKeys.length; j++){

                neighbour_j = _neighbours[commonNeighbourKeys[j]];
                id_j = neighbour_j.id;

                // skip self or any new neighbours
                if (id_j === _self.id || newNeighbourKeys.includes(commonNeighbourKeys[j])){
                    continue;
                }
            ...
            // add the new neighbour to the list of recipients
