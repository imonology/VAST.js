...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    this.init = function (self_id, port, localIP, onDone) {
    ...
    var _join = this.join = function (GW_addr, aoi, onDone, onMove) {
        ...
        if (_getID() === VAST.ID_UNASSIGNED) {
            LOG.debug(VON_Message.VON_HAIL);
            _sendMessage(VAST.ID_GATEWAY, VON_Message.VON_HAIL, {request: true}, VAST.priority.HIGHEST);
        }
        else
            _setInited();
    }
    ...
    var _ping = function() {
        ...
        for (var key in _neighbors)
            _sendMessage(key, VON_Message.VON_PING, {}, VAST.priority.HIGH, true);
    ...
    var _calcLatency = function (pack) {
        ...
        if (pack.latency >= 0) {
            ...
            // forwarding latency
            LOG.error(pack.forwardTime-pack.receiveTime, _self.id);
        ...
        switch (pack.type) {
            ...
            case VON_Message.VON_HAIL: {
                ...
                if (typeof pack.msg.request !== 'undefined' && pack.msg.request == true) {
                    ...
                    _sendMessage(from_id, VON_Message.VON_HAIL, {request: false, aid: remote_id}, VAST.priority.HIGH, true);
                }
            ...
            case VON_Message.VON_JOIN: {
                ...
                if (_isOverlapped(joiner.aoi.center, joiner.id)) {
                    ...
                    _updateNode(joiner);
                    _sendMessage(joiner.id, VON_Message.VON_POS, {pos: joiner.aoi.center}, VAST.priority.HIGH, true);
                    break;
                }
                */
            ...
            case VON_Message.VON_POS: {
                ...
                _sendMessage(from_id, VON_Message.VON_JOIN, _self, VAST.priority.HIGH, true);
            }
    ...
    var _storeMapping, _getID, _setID, _disconnect, _sendMessage, _sendPack, _visualReturn;
