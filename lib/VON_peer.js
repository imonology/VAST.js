...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    var _move = this.move = function (aoi, sendtime) {
        ...
        var checkPos = new VAST.pos(aoi.center.x, aoi.center.y);

        if (_isOverlapped(checkPos, _self.id)) {
            LOG.debug("Overlap detected");
            aoi.center = _adjustPos(checkPos, _self.id);
    ...
    LOG.info('outside VON Peer called');
    //
    // node management
    //
    ...
    var _adjustPos = function (pos, id) {
        do {
            // adjust position randomly between -0.5 and 0.5
