...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    var _move = this.move = function (aoi, sendtime) {
        LOG.warn('VON_Peer ['+_self.id+'] moving to: ' + aoi.center.toString());
        ...
        LOG.debug('AoI + center');
        LOG.debug(_self.aoi);
        LOG.debug(_self.aoi.center);
        ...
        LOG.warn("Regular neighbours:");
        LOG.warn(regular_list);
        pack.targets = regular_list;
        pack.type = (aoi_reshaped ? VON_Message.VON_MOVE_F : VON_Message.VON_MOVE);
        _sendPack(pack, true);

        LOG.warn("Boundary Neighbours:");
        LOG.warn(boundary_list);
    ...
    var _adjustPos = function (pos, id) {
        do {
            ...
            pos = _bound(pos);
            LOG.debug("Position adjusted to ("+pos.x+","+pos.y+")");
    ...
    var _insertNode = function (node) {
        ...
        LOG.info('node id:'+node.id+' node address:'+node.endpt.addr);
        ...
        if (_isOverlapped(node.aoi.center,node.id)) {
            LOG.info("Adjusting local position for client [" + node.id + "] before inserting into voronoi");
    ...
    var _setInited = function () {
        ...
        LOG.info('_self.id:'+_self.id+' ID_GATEWAY:'+VAST.ID_GATEWAY);
        if (_self.id === VAST.ID_GATEWAY)
            _setJoined();
        else
    ...
    var _bound = function (pos) {
        ...
        if (pos.x < bbox.xl)         pos.x = bbox.xl;
        if (pos.y < bbox.yt)         pos.y = bbox.yt;
        if (pos.x > bbox.xr)         pos.x = bbox.xr;
        if (pos.y > bbox.yb)         pos.y = bbox.yb;
        return pos;
    }
    ...
    var _packetHandler = this.packetHandler = function (from_id, pack) {
        ...
        if (!progress) {
            LOG.warn("message was stored for later")
            messages.push(obj);
            _visualReturn("empty", false);
            return true;
        } else {
            LOG.warn("Processing message");
        }
        ...
        switch (pack.type) {
            ...
            case VON_Message.VON_JOIN: {
                ...
                if (_isOverlapped(joiner.aoi.center, joiner.id)) {
                    LOG.debug("joiner client overlap. Adjusting position");
                ...
                for (var id in _neighbors) {
                    ...
                    LOG.debug('neighbour.id: '+neighbor.id);
                    LOG.debug('joiner.id: '+joiner.id);
            ...
            case VON_Message.VON_POS: {
                ...
                var pos = new VAST.pos();
                pos.parse(pack.msg.pos);
                LOG.info("changing position to ("+pos.x+','+pos.y+")");
            ...
            case VON_Message.VON_MOVE_FB: {
                ...
                if (_isOverlapped(node.aoi.center, node.id)) {
                    LOG.debug("Position being adjusted locally to avoid overlap");
                    node.aoi.center.update(_adjustPos(node.aoi.center, node.id));
                }
            ...
            case VON_Message.VON_BYE: {
                ...
                LOG.debug('disconnect success: ' + result);
    ...
    var _visualComm = this.visualComm = function (type,message) {
        ...
        switch (type) {
            ...
            case "voro": {
                if (_visual_id == undefined) {
                    LOG.info("Visualiser tick has started");
            ...
            case "voro_client": {
                if (_global_id == undefined) {
                    LOG.info("Global view tick has started");
                    LOG.warn(_voro.get(_id));
            ...
            case "sync_move": {
                LOG.info("Start synchronised movement");
            ...
            case "disconnect": {
                LOG.debug("Visualiser tick stopped");
                clearInterval(_visual_id);
                _visual_id = undefined;
            ...
            default: {
                LOG.error("VON_peer: visual communicator does not understand type '"+type+"'");
            }
            break;
        }
    }
    ...
    var _time_drop;
    var newFileTime = new Date();
    newFileTime = newFileTime.getTime();
