...
function VONPeer(l_aoi_buffer, l_aoi_use_strict) {
    ...
    var _move = this.move = function (aoi, sendtime) {
        ...
        var state = new VAST.clientState(_self.id,UTIL.getTimestamp(), _self.aoi.center,neighbour_pos, latMax, latMin, latAve, bandwidth);
    ...
    var _updateNode = function (node) {
        ...
        LOG.debug("Updating voronoi diagram for " + node.id + " to (" + node.aoi.center.x + "," + node.aoi.center.y + ")");

        _voro.update(node.id, node.aoi.center);

        LOG.debug("Updating neighbour list for " + node.id + " to (" + node.aoi.center.x + "," + node.aoi.center.y + ")");
        LOG.warn("Neighbor before update");
        LOG.debug(_neighbors[node.id]);
        LOG.debug(node);

        _neighbors[node.id].update(node);

        LOG.warn("Neighbor update:");
        LOG.debug(_neighbors[node.id]);
        LOG.debug(node);
        ...
        switch (type) {
            ...
            case "voro_client": {
                if (_global_id == undefined) {
                    ...
                    _global_id = setInterval(function(voro, neighbours, id) {
                        _visualReturn(type, [voro.get(id), neighbours, id]);
                    }, MOVEMENT_GLOBAL, _voro, _neighbors, _id);
                }
            }
            break;
