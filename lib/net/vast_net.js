...
//
function vast_net(onReceive, onConnect, onDisconnect, visualComm, id) {
    ...
    var l_connect = function (id) {
        ...
        var conn = new l_net(
            ...
            function (socket) {
                ...
                //console.log('id: ' + id + ' disconnected from '+_self_id);
            ...
            // error callback
            function (socket) {
                LOG.error('id: ' + id + ' erraneously disconnected from ' + _self_id);
    ...
    var l_send = this.send = function (id_list, pack, is_reliable, onDone) {
        ...
        pack.src = _self_id;
        LOG.warn("Assign pack id: "+pack.src+" _self_id");
        ...
        for (var i=0; i < id_list.length; i++) {
            ...
            if (_msgqueue.hasOwnProperty(id) === false) {
				LOG.warn('msgqueue for [' + id + '] does not exist, create one...');
            ...
            if (_sockets.hasOwnProperty(id)) {
				...
				LOG.warn('socket for [' + id + '] does not exist, try to connect...');
                l_connect(id);
            }
        }
    ...
    this.listen = function (port, onDone) {
        ...
        _server = new l_net(
            ...
            // error callback
            function (socket) {
                LOG.error('id: ' + id + ' disconnected from ' + _self_id);
	...
	var _processData = function (socket, data) {
        ...
        while (true) {
            ...
            if (previous_pack != undefined) {
                if (pack.type == previous_pack.type &&
                    pack.src == previous_pack.src &&
                    (pack.time-previous_pack.time) <= 100) {
                        for (var keys in pack.targets) {
                            if (pack.targets[keys] == previous_pack.targets[keys]) {
                                console.log("Duplicate packet identified: src: " + pack.src + " type: " + VON_Message_String[pack.type] + " targets: " + pack.targets + " time: " + (pack.time-previous_pack.time));
                                continue;
                            }
                        }
                    }
