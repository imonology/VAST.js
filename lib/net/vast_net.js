...
function vast_net(onReceive, onConnect, onDisconnect, visualComm, id) {
    ...
    this.listen = function (port, onDone) {
        ...
        _server = new l_net(
            ...
            function (socket) {
                //('disconnet occur');
                //console.log('id: ' + id + ' disconnected from ' + _self_id + " with socket ID of " + socket.id + " and socket size of " + Object.keys(_sockets).length);
            ...
            // error callback
            function (socket) {
                LOG.error('id: ' + id + ' disconnected from ' + _self_id + " with socket ID of " + socket.id);
	...
	var _processData = function (socket, data) {
        ...
        while (true) {
            ...
            if (socket.hasOwnProperty('id') === false) {
                LOG.warn('socket has no id assigned yet...assigning: ' + pack.src);
