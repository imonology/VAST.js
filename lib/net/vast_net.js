...
//
function vast_net(onReceive, onConnect, onDisconnect, visualComm, clientComm, id, l_local_IP, l_entry_servers) {
    ...
    var l_send = this.send = function (id_list, pack, is_reliable) {
    ...
    this.listen = function (port, onDone) {
        ...
        _server = new l_net(
            ...
            function (type,request, handler) {
                // forward request up a level
                visualComm(type,request, handler);
            },

            // communication channel for client listener
            function(type, data) {
                // forward communication to matcher
                LOG.debug("type: " + type);
                LOG.debug("Data: ");
                LOG.debug(data);
    ...
    // return channel for data for vast clients
    var _clientReturn = this.clientReturn = function (type, data) {
        _server.clientReturn(type,data);
    }
	...
	var _processData = function (socket, data) {
        ...
        while (true) {
            ...
            // check for a VON_PING, in which case return VON_PONG immediately
            if (pack.type == VON_Message.VON_PING) {
                LOG.debug("Ping received from [" + pack.src + "]",_self_id);
                var pong = new VAST.pack (
                    VON_Message.VON_PONG,
                    {sendTime: pack.sendTime},
                    VAST.priority.HIGHEST,
                    pack.layer,
                    _self_id
                );
                pong.targets.push(pack.src)
                l_send(pong.targets, pong, true);
                continue;
            }

            if (pack.type == VON_Message.VON_PONG) {
                pack.ping = UTIL.getTimestamp()-pack.msg.sendTime;
                LOG.info("Pong received with a ping of " + pack.ping,_self_id);
            }
            ...
            // TODO: change latency calc to ping-pong packet
            pack.receiveTime = now;

            pack.latency = pack.receiveTime-pack.sendTime;
            ...
            if (_previous_pack != undefined) {
                if (pack.type == _previous_pack.type &&
                    ...
                    (pack.receiveTime-_previous_pack.receiveTime) <= 100 &&
                    pack.type != VON_Message.VON_PONG &&
                    pack.layer == _previous_pack.layer &&
                    pack.layer != LAYER.MATCHER) {
                        for (var keys in pack.targets) {
                            if (pack.targets[keys] == _previous_pack.targets[keys]) {
                                ...
                                LOG.debug("Duplicate packet identified: src: " + pack.src
                                    ...
                                    + " time: " + (pack.receiveTime-_previous_pack.receiveTime)
                                    + " msg: " + pack.msg
                                    + " layer: " + pack.layer
                                );
                                /*
