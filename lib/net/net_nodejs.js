...
function net_nodejs(onReceive, onConnect, onDisconnect, onError, visualComm) {
    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            // setup the global viewers definitions
            _view.on('connect', function() {
                // check for whether the socket is already connected
                LOG.debug("Socket.io has connected with the global visualiser");
                _view.emit("connected", "Client: ["+id+"] connected");
            });
