    ...
    this.clientReturn = function (type, data) {
        ...
        switch (type) {
            case "type": {
                ...
                if (socket != undefined) {
                    socket.emit(type, data.connectionID);
                } else {
            ...
            case "join": {
                ...
                if (socket != undefined) {
                    LOG.layer("net_nodejs::join => returning data to " + data.username, _self_id);
                    socket.emit(type, data.username, _clientID);
                } else {
            ...
            case "publish": {
                ...
                if (socket != undefined) {
                    socket.emit("publication", data.clientID, payload.username, payload.aoi.center.x, payload.aoi.center.y, payload.aoi.radius, payload.payload, payload.channel);
                } else {
