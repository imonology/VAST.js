    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            if (_entryServer === undefined) {
                ...
                _entryServer.on("subscribe", function (clientID, channel, username, x, y, AoI){
                    LOG.layer("subscribe received: " + clientID + " " + channel + " " + username + " " + x + " " + y + " " + AoI);
                    var data = {
                        clientID: clientID,
                        channel: channel,
    ...
    this.clientReturn = function (type, data) {
        ...
        switch (type) {
            ...
            case "publish": {
                ...
                if (_entryServer != undefined) {
                    _entryServer.emit("matcherPub", data.clientID, payload.username, payload.aoi.center.x, payload.aoi.center.y, payload.aoi.radius, payload.payload, payload.channel);
                } else {
