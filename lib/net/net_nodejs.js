...
function net_nodejs(id, onReceive, onConnect, onDisconnect, onError, visualComm) {
    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            _server = l_net.createServer(function() {
                // store port in server object
                _server.port = port;
            ...
            _io.on('connection', function(visualiser){
                ...
                _sockIO.on("next", function(bool){
                    if (bool) {
                        LOG.debug("Move on to next command");
                    } else {
                        LOG.debug("Switch debug mode");
                    }
            ...
            _http.on('error', function (e) {
                if (e.code === 'EADDRINUSE') {
                    LOG.debug("Socket.io port "+sockPort+" in use, retrying next port.");
                    sockPort++
                } else {
            ...
            _server.on('error', function (e) {
                ...
                LOG.debug('net_nodejs: listen error caught: ' + e);
            ...
            if (_view === undefined) {
                ...
                _view = ioClient('http://127.0.0.1:7777');
	...
	var setupSocket = function (socket) {
        ...
        if (typeof onConnect === 'function'){
            ...
            console.log('onConnect called in '+_self_id);
            onConnect(socket);
        }
        ...
        socket.on('close', function (has_error) {
            ...
            if (has_error)
                ...
                console.log('closing socket in ' + _self_id);
            }
