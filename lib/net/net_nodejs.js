...
function net_nodejs(onReceive, onConnect, onDisconnect, onError, visualComm, id) {
    ...
    this.visualReturn = function (data) {
        if (_io !== undefined) {
            ...
            // temporary way of checking what type of message it is
            if (data.indexOf('assigned_id') != -1){
                console.log("Id sent:"+data[12]);
                _io.emit("id", data[12]);
            } else {
                _io.emit("voro", data);
            }
