...
function net_nodejs(id, onReceive, onConnect, onDisconnect, onError, visualComm, clientComm) {
    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            // setup the definitions for the client socket.io event handlers
            _ioClient.on('connection', function(socket) {
                ...
                socket.on("type", function (type) {
                    ...
                    // temp socket storage
                    _id2socket[socket.id] = socket;

                    //
                    clientComm("type",socket.id);
                });

                socket.on("join", function (username) {
                    LOG.layer("net_nodejs::join => username received from " + username);
                    _id2socket[socket.id] = socket;
                    var data = {
                        username: username,
                        socketID: socket.id
                    }
                    _clientComm(type, username);
                });

                socket.on("subscribe", function (channel, username, x, y, AoI){
                    LOG.layer("subscribe received: "+ channel +" " + username + " " + x + " " + y + " " + AoI);
                    var data = {
                        type: _id2socket[socket.id],
                        channel: channel,
                        username: username,
                        x: x,
                        y: y,
                ...
                socket.on("unsubscribe", function (channel, username) {
                    LOG.layer("net_nodejs::vast => unsubscribe received: " + channel + " " + username);
                    var data = {
                        type: _id2socket[socket.id],
                        channel: channel,
                        username: username
                ...
                socker.on("publish", function (connectionID, username, x, y, radius, payload, channel, packetName) {
                    LOG.layer("net_nodejs::publish => received publish from " + username + " " + connectionID);
                    var data = {
                        clientID: _id2socket[connectionID],
                        username: username,
                        x: x,
                        y: y,
                        radius: radius,
    ...
    this.clientReturn = function (type, data) {
        ...
        switch (type) {
            case "type": {
                LOG.layer("net_nodejs::clientReturn => sending connection acknowledgement to server");

                // retrieve socket through which communication goes through
                var socket = _id2socket[data.socketID];

                socket.emit(type, data.serverID);
            }
                break;
            case "join": {
