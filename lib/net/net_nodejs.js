...
function net_nodejs(id, numEntry, onReceive, onConnect, onDisconnect, onError, visualComm, clientComm) {
    ...
    // entry server socket
    //var _entryServerSock = undefined;
    ...
    // number of entry servers
    var _numEntry = numEntry || 1;
    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            _io.on('connection', function(visualiser){
                ...
                // request site map from VON peer and matcher in upper levels
                _sockIO.on('voro', function(message, onDone){
                    console.log("received voro")
                    visualComm("voro",message, LAYER.VON);
                    visualComm("voro", message, LAYER.MATCHER)
                });
            ...
            _ioClient.on('connection', function(socket) {
                ...
                socket.on("join", function (username, type, clientID, aoi, newConnection) {
            ...
            if (_view === undefined) {
                ...
                _view = ioClient('http://10.110.117.14:7777');

                // start synchronised movement
                _view.on("move", function(time) {
                    visualComm("sync_move", true,LAYER.VON);
                })
            }
            */
        }
    ...
    this.initialiseEntry = function() {
         LOG.debug("Starting entry server creation");
        //for (var i = 0; i < _numEntry; i++) {
            LOG.debug("Creating entry " + 1);
            // create connection to entry servers
            if (!entryServers.hasOwnProperty(0) || !entryServers.hasOwnProperty(1)) {
                // NOTE: change this in order to adjust connection to entry server
                LOG.debug("Connecting to ES at port " + (2999));
                var _entryServer1 = ioClient('http://10.110.117.14:2999', {upgrade: false, transports: ['websocket']});
                entryServer[0] = false;
                LOG.debug("Connecting to ES at port " + (2998));
                var _entryServer2 = ioClient2('http://10.110.117.14:2998', {upgrade: false, transports: ['websocket']});
                entryServer[1] = false;

                _entryServer1.on("connected", function() {
                    LOG.debug("Connected 1");
                    LOG.debug(entryServer);
                    if (!entryServer[0]) {
                        storeSocket(1,_entryServer1);

                        LOG.debug("Socket id: "  + _entryServer1.id);

                         _client2ES[_self_id] = _entryServer1.id;

                        entryServers.push(_entryServer1.id);
                ...
                _entryServer1.on("type", function (type) {
                    LOG.layer("net_nodejs::type => received type: " + type, _self_id);
                    // insert socket into temporary view in case it isn't stored already
                    if (!_id2socket.hasOwnProperty(socket.id)) {
                        LOG.layer("net_nodejs::type => storing temporary socket");
                ...
                _entryServer1.on("join", function (username, type, clientID, aoi, newConnection) {
                    LOG.layer("net_nodejs::join => username received from " + username + " (" + type + ")", _self_id);
                    var data = {
                        username: username,
                       // socketID: _entryServer1.id,
                        type: type,
                        clientID: clientID,
                        aoi: aoi,
                        newConnection: newConnection
                    ...
                    // setting client ID to ES link 
                    _client2ES[clientID] = 1;

                    LOG.layer("net_nodejs::join => client2ES");
                    LOG.layer(_client2ES);
                ...
                _entryServer1.on("subscribe", function (clientID, channel, username, x, y, AoI, senderID){
                    LOG.layer("subscribe received: " + clientID + " " + channel + " " + username + " " + x + " " + y + " " + AoI);
                    var data = {
                        clientID: clientID,
                        channel: channel,
                ...
                _entryServer1.on("unsubscribe", function (clientID,channel, username) {
                    LOG.layer("net_nodejs::vast => unsubscribe received: " + channel + " " + username + " " + clientID, _self_id);
                    var data = {
                        clientID: clientID,
                        channel: channel,
                ...
                _entryServer1.on("move", function (connectionID, username, x, y, radius, payload, channel, packetName, matcherID) {
                    LOG.layer("net_nodejs::move => [" + connectionID + "] moving " + username + " to <" + x + "," + y + "> on channel " + channel, _self_id);
                    var data = {
                        clientID:   connectionID,
                        username:   username,
                ...
                _entryServer1.on("matcherMove", function(pos){
                    LOG.layer("net_nodejs::matcherMove => Moving matcher to <" + pos.x + "," + pos.y + ">");

                    for (var key in entryServers) {
                        if (entryServers[key].id != _entryServer.id) 
                            entryServers[key].emit("inform_move", _self_id, pos);
                    }

                    clientComm("matcherMove", pos);
                });

                _entryServer1.on("publish", function (clientID, username, x, y, radius, payload, channel, packetName) {
                    LOG.layer("net_nodejs::publish => received publish from " + username + " " + clientID, _self_id);
                    LOG.layer(clientID);
                    var data = {
                        clientID: clientID,
                ...
                _entryServer1.on("leave", function(clientID) {
                    LOG.layer("net_nodejs::leave => client " + clientID + " is leaving the system");

                    clientComm("leave", clientID);
                });

                _entryServer1.on("disconnect", function (reason, id) {
                    LOG.layer("net_nodejs:disconnect => entry server disconnected with reason: " + reason);
                    _entryServer1.close();
                    _entryServer1 = undefined;

                    //clientComm("disconnect", id);
                });

                _entryServer1.on("error", function (error) {
                    LOG.layer("net_nodejs::error => entry server encountered error: " + error);
                ...
                _entryServer2.on("connect", function() {
                    LOG.debug("Connected 2");
                    LOG.debug(entryServer);
                    if (!entryServer[1]) {
                        storeSocket(2,_entryServer2);

                        LOG.debug("Socket id: "  + _entryServer2.id);

                        entryServers.push(_entryServer2.id);
                ...
                _entryServer2.on("type", function (type) {
                    LOG.layer("net_nodejs::type => received type: " + type, _self_id);
                    // insert socket into temporary view in case it isn't stored already
                    if (!_id2socket.hasOwnProperty(socket.id)) {
                        LOG.layer("net_nodejs::type => storing temporary socket");
                ...
                _entryServer2.on("join", function (username, type, clientID, aoi, newConnection) {
                    LOG.layer("net_nodejs::join => username received from " + username + " (" + type + ")", _self_id);
                    var data = {
                        username: username,
                        socketID: 2,
                ...
                _entryServer2.on("subscribe", function (clientID, channel, username, x, y, AoI, senderID){
                    LOG.layer("subscribe received: " + clientID + " " + channel + " " + username + " " + x + " " + y + " " + AoI);
                    var data = {
                        clientID: clientID,
                        channel: channel,
                ...
                _entryServer2.on("unsubscribe", function (clientID,channel, username) {
                    LOG.layer("net_nodejs::vast => unsubscribe received: " + channel + " " + username + " " + clientID, _self_id);
                    var data = {
                        clientID: clientID,
                        channel: channel,
                ...
                _entryServer2.on("move", function (connectionID, username, x, y, radius, payload, channel, packetName, matcherID) {
                    LOG.layer("net_nodejs::move => [" + connectionID + "] moving " + username + " to <" + x + "," + y + "> on channel " + channel, _self_id);
                    var data = {
                        clientID:   connectionID,
                        username:   username,
                ...
                _entryServer2.on("matcherMove", function(pos){
                    LOG.layer("net_nodejs::matcherMove => Moving matcher to <" + pos.x + "," + pos.y + ">");
                ...
                _entryServer2.on("publish", function (clientID, username, x, y, radius, payload, channel, packetName) {
                    LOG.layer("net_nodejs::publish => received publish from " + username + " " + clientID, _self_id);
                    LOG.layer(clientID);
                    var data = {
                        clientID: clientID,
                ...
                _entryServer2.on("leave", function(clientID) {
                    LOG.layer("net_nodejs::leave => client " + clientID + " is leaving the system");

                    clientComm("leave", clientID);
                });

                _entryServer2.on("disconnect", function (reason, id) {
                    LOG.layer("net_nodejs:disconnect => entry server disconnected with reason: " + reason);
                    _entryServer2.close();
                    _entryServer2 = undefined;

                    //clientComm("disconnect", id);
                });

                _entryServer2.on("error", function (error) {
                    LOG.layer("net_nodejs::error => entry server encountered error: " + error);
    ...
    this.visualReturn = function (type,data) {
        ...
        if (_view !== undefined && (type == "voro_client" || type == "matcher" || type == "matcher_leave")) {
    ...
    this.clientReturn = function (type, data) {
        ...
        switch (type) {
            case "ID": {
                ...
                if (_id2socket.hasOwnProperty(data.socketID)) {
                    _id2socket[data.socketID].emit(type, data.id, data.position);

                    entryServer[data.num] = true;
                } else {
                    LOG.layer("net_nodejs::type => entryServer " + data.socketID + " does not exist. Cannot send 'ID' acknowledgement", _self_id);                    
                }
            }
                break;
            ...
            case "type": {
                ...
                var _entryServer = _id2socket[_client2ES[data.connectionID]];
            ...
            case "join": {
                ...
                LOG.layer("checking IDs and sockets and stuff");
                LOG.layer(_clientID);
                LOG.layer(_client2ES);

                var _entryServer = _id2socket[_client2ES[_clientID]];

                // emit 'join' event with above data
                if (_entryServer != undefined) {
                    LOG.layer("net_nodejs::join => returning data to " + data.username + " id: " + _clientID, _self_id);
                    _entryServer.emit("matcherJoin", _self_id, data.username, _clientID, data.newConnection);
                } else {
            ...
            case "publish": {
                ...
                var _entryServer = _id2socket[_client2ES[data.clientID]];
            ...
            case "clientTransfer": {
                LOG.layer("net_nodejs::clientTransfer => transfer " + data.clientID + " to matcher " + data.matcherID, _self_id);

                var _entryServer = _id2socket[_client2ES[data.clientID]];

                if (_entryServer != undefined) {
                    _entryServer.emit("clientTransfer", data.clientID, data.matcherID);
                } else {
                     LOG.layer("net_nodejs::clientTransfer => entryServer does not exist. Cannot initiate client transfer. Need to reinitialise client", _self_id);
                }
            }
                break;

            case "help": {
                LOG.layer("net_nodejs::help => sending out request for help", _self_id);

                var _entryServer = _id2socket[_client2ES[data.matcherID]];

                if (_entryServer !== undefined) {
                    _entryServer.emit(type,data.matcherID, data.aoi, data.en_list);
                } else {
                    LOG.layer("net_nodejs::help => socket does not exist. Ignore request for help", _self_id);
                }
            }
                break;

            case "normalise" : {
                LOG.debug("net_nodejs::normalise => Asking ES to return matchers to original positions");

                var _entryServer = _id2socket[_client2ES[data.id]];

                if (_entryServer !== undefined) {
                    _entryServer.emit(type,data.id, data.neighbours);
                } else {
                    LOG.layer("net_nodejs::normalise => socket does not exist. Ignore request for normalisation", _self_id);
                }
            }
                break;

            case "move_matcher": {
                LOG.debug("net_nodejs::move_matcher => asking neighbour matcher to move a minimum distance away from me");

                var _entryServer = _id2socket[_client2ES[data.id]];

                if (_entryServer !== undefined) {
                    _entryServer.emit(type,data.id, data.matcherID, data.min);
                } else {
                    LOG.layer("net_nodejs::move_matcher => socket does not exist. Ignore request for moving neighbour matcher", _self_id);
                }
            }
                break;
            default:
                LOG.error("net_nodejs::clientReturn => unknown type. Ignore.");
        }
    }
