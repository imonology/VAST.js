...
function net_nodejs(id, onReceive, onConnect, onDisconnect, onError, visualComm, clientComm) {
    ...
    var _vastServer, _vastClient = undefined;
    ...
    this.listen = function (port, onDone) {
        ...
        try {
            ...
            _ioClient.on('connection', function(socket) {
                ...
                // get the type of the connection (server or client)
                socket.emit("type",false);

                socket.on("type", function (type) {
                    LOG.layer("clientSocket => received type: " + type, _self_id);
                    if (type == "server") {
                        _vastServer = socket;
                        typeVAST[socket.id] = "server";
                    } else {
                        _vastClient = socket;
                        typeVAST[socket.id] = "client";
                    }
                    clientComm("type",type);
                });

                socket.on("subscribe", function (channel, username, x, y, AoI){
                    LOG.layer("subscribe received: "+ channel +" " + username + " " + x + " " + y + " " + AoI);
                    var data = {
                        type: typeVAST[socket.id],
                        channel: channel,
                        username: username,
                        x: x,
                        y: y,
