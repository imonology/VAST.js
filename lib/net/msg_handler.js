...
function msg_handler(l_self_id, l_listen_port, l_onDone) {
    ...
    // local IP & port
    var _localAddress = {host: '10.10.11.179', port: 0};
    ...
    var _visualComm = function (type,message) {
        // TODO: handle sending to all handlers
        if (_handlers[0] !== undefined){
            _handlers[0].visualComm(type,message);
        }
    }
    ...
    _net = new VAST.net(_packetHandler, _connHandler, _disconnHandler, _visualComm, l_self_id);
